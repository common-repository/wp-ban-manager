 ...
 * @package            WPBMAdminPageFramework
 * @subpackage        Utility
 * @internal
 */
if (!class_exists('WPBMAdminPageFramework_Debug')) : class WPBMAdminPageFramework_Debug
{
	...
	static public function logArray($asArray, $sFilePath = null)
	{
		...
		file_put_contents($sFilePath ? $sFilePath : dirname(__FILE__) . '/array_log.txt', date("Y/m/d H:i:s", current_time('timestamp')) . ' ' . "{$_iPageLoadID} {$sCallerClasss}::{$sCallerFunction} " . WPBMAdminPageFramework_Utility::getCurrentURL() . PHP_EOL . print_r($asArray, true) . PHP_EOL . PHP_EOL, FILE_APPEND);
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_HelpPane_Base')) : abstract class WPBMAdminPageFramework_HelpPane_Base extends WPBMAdminPageFramework_Debug
{
	protected $_oScreen;
...
if (!class_exists('WPBMAdminPageFramework_HelpPane_MetaBox')) : class WPBMAdminPageFramework_HelpPane_MetaBox extends WPBMAdminPageFramework_HelpPane_Base
{
	...
	public function _replyToRegisterHelpTabTextForMetaBox()
	{
		...
		if (isset($_GET['post'], $_GET['action']) && !in_array(get_post_type($_GET['post']), $this->oProp->aPostTypes)) return;
		$this->_setHelpTab($this->oProp->sMetaBoxID, $this->oProp->sTitle, $this->oProp->aHelpTabText, $this->oProp->aHelpTabTextSide);
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_HelpPane_Page')) : class WPBMAdminPageFramework_HelpPane_Page extends WPBMAdminPageFramework_HelpPane_Base
{
...
if (!class_exists('WPBMAdminPageFramework_HelpPane_TaxonomyField')) : class WPBMAdminPageFramework_HelpPane_TaxonomyField extends WPBMAdminPageFramework_HelpPane_MetaBox
{
	public function _replyToRegisterHelpTabTextForMetaBox()
	{
		$this->_setHelpTab($this->oProp->sMetaBoxID, $this->oProp->sTitle, $this->oProp->aHelpTabText, $this->oProp->aHelpTabTextSide);
...
if (!class_exists('WPBMAdminPageFramework_PageLoadInfo_Base')) : abstract class WPBMAdminPageFramework_PageLoadInfo_Base
{
	...
	public function _replyToGetPageLoadInfo($sFooterHTML)
	{
		...
		$sOutput = "<div id='admin-page-framework-page-load-stats'>" . "<ul>" . "<li>" . sprintf($this->oMsg->__('queries_in_seconds'), $nQueryCount, $nSeconds) . "</li>" . "<li>" . sprintf($this->oMsg->__('out_of_x_memory_used'), $nMemoryUsage, $nMemoryLimit, round(($nMemoryUsage / $nMemoryLimit), 2) * 100 . '%') . "</li>" . "<li>" . sprintf($this->oMsg->__('peak_memory_usage'), $nMemoryPeakUsage) . "</li>" . "<li>" . sprintf($this->oMsg->__('initial_memory_usage'), $sInitialMemoryUsage) . "</li>" . "</ul>" . "</div>";
...
if (!class_exists('WPBMAdminPageFramework_PageLoadInfo_Page')) : class WPBMAdminPageFramework_PageLoadInfo_Page extends WPBMAdminPageFramework_PageLoadInfo_Base
{
	...
	public static function instantiate($oProp, $oMsg)
	{
		...
		self::$_oInstance = new WPBMAdminPageFramework_PageLoadInfo_Page($oProp, $oMsg);
	...
	{
		$sCurrentPageSlug = isset($_GET['page']) ? $_GET['page'] : '';
		if ($this->oProp->isPageAdded($sCurrentPageSlug)) add_filter('update_footer', array($this, '_replyToGetPageLoadInfo'), 999);
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_PageLoadInfo_PostType')) : class WPBMAdminPageFramework_PageLoadInfo_PostType extends WPBMAdminPageFramework_PageLoadInfo_Base
{
	private static $_oInstance;
	private static $aClassNames = array();
	...
	public static function instantiate($oProp, $oMsg)
	{
		if (in_array($oProp->sClassName, self::$aClassNames)) return self::$_oInstance;
		self::$aClassNames[] = $oProp->sClassName;
		self::$_oInstance = new WPBMAdminPageFramework_PageLoadInfo_PostType($oProp, $oMsg);
		return self::$_oInstance;
	}
	...
	public function _replyToSetPageLoadInfoInFooter()
	{
		...
		if (isset($_GET['post_type'], $this->oProp->sPostType) && $_GET['post_type'] == $this->oProp->sPostType || WPBMAdminPageFramework_WPUtility::isPostDefinitionPage($this->oProp->sPostType)) add_filter('update_footer', array($this, '_replyToGetPageLoadInfo'), 999);
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_WalkerTaxonomyChecklist')) : class WPBMAdminPageFramework_WalkerTaxonomyChecklist extends Walker_Category
{
	function start_el(&$sOutput, $oCategory, $iDepth = 0, $aArgs = array(), $iCurrentObjectID = 0)
	{
		...
		$aInputAttributes = isset($aInputAttributes[$iID]) ? $aInputAttributes[$iID] + $aArgs['attributes'] : $aArgs['attributes'];
		$aInputAttributes = array('id' => $sID, 'value' => 1, 'type' => 'checkbox', 'name' => "{$aArgs['name']}[{$iID}]", 'checked' => in_array($iID, ( array )$aArgs['selected']) ? 'Checked' : '',) + $aInputAttributes;
		$sOutput .= "\n" . "<li id='list-{$sID}' class='category-list'>" . "<label for='{$sID}' class='taxonomy-checklist-label'>" . "<input value='0' type='hidden' name='{$aArgs['name']}[{$iID}]' />" . "<input " . WPBMAdminPageFramework_WPUtility::generateAttributes($aInputAttributes) . " />" . esc_html(apply_filters('the_category', $oCategory->name)) . "</label>";
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_FieldTypeRegistration')) : class WPBMAdminPageFramework_FieldTypeRegistration
{
	...
	function __construct(&$aFieldTypeDefinitions, $sExtendedClassName, $oMsg)
	{
		foreach (self::$aDefaultFieldTypeSlugs as $sFieldTypeSlug) {
			$sInstantiatingClassName = "WPBMAdminPageFramework_FieldType_{$sFieldTypeSlug}";
			if (class_exists($sInstantiatingClassName)) {
				$oFieldType = new $sInstantiatingClassName($sExtendedClassName, null, $oMsg, false);
				foreach ($oFieldType->aFieldTypeSlugs as $sSlug) $aFieldTypeDefinitions[$sSlug] = $oFieldType->getDefinitionArray();
			}
		}
	}
	...
	static public function _setFieldHeadTagElements(array $aField, $oProp, $oHeadTag)
	{
		...
		static $aLoadFlags = array();
		$aLoadFlags[$oProp->_sPropertyType] = isset($aLoadFlags[$oProp->_sPropertyType]) && is_array($aLoadFlags[$oProp->_sPropertyType]) ? $aLoadFlags[$oProp->_sPropertyType] : array();
		...
		if (is_callable($oProp->aFieldTypeDefinitions[$sFieldType]['hfFieldSetTypeSetter'])) call_user_func_array($oProp->aFieldTypeDefinitions[$sFieldType]['hfFieldSetTypeSetter'], array($oProp->_sPropertyType));
		if (is_callable($oProp->aFieldTypeDefinitions[$sFieldType]['hfFieldLoader'])) call_user_func_array($oProp->aFieldTypeDefinitions[$sFieldType]['hfFieldLoader'], array());
		if (is_callable($oProp->aFieldTypeDefinitions[$sFieldType]['hfGetScripts'])) $oProp->sScript .= call_user_func_array($oProp->aFieldTypeDefinitions[$sFieldType]['hfGetScripts'], array());
		if (is_callable($oProp->aFieldTypeDefinitions[$sFieldType]['hfGetStyles'])) $oProp->sStyle .= call_user_func_array($oProp->aFieldTypeDefinitions[$sFieldType]['hfGetStyles'], array());
		if (is_callable($oProp->aFieldTypeDefinitions[$sFieldType]['hfGetIEStyles'])) $oProp->sStyleIE .= call_user_func_array($oProp->aFieldTypeDefinitions[$sFieldType]['hfGetIEStyles'], array());
		foreach ($oProp->aFieldTypeDefinitions[$sFieldType]['aEnqueueStyles'] as $asSource) {
			if (is_string($asSource)) $oHeadTag->_forceToEnqueueStyle($asSource); else if (is_array($asSource) && isset($asSource['src'])) $oHeadTag->_forceToEnqueueStyle($asSource['src'], $asSource);
		}
		foreach ($oProp->aFieldTypeDefinitions[$sFieldType]['aEnqueueScripts'] as $asSource) {
			if (is_string($asSource)) $oHeadTag->_forceToEnqueueScript($asSource); else if (is_array($asSource) && isset($asSource['src'])) $oHeadTag->_forceToEnqueueScript($asSource['src'], $asSource);
		}
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_HeadTag_Base')) : abstract class WPBMAdminPageFramework_HeadTag_Base
{
	...
	{
		$this->oProp = $oProp;
		$this->oUtil = new WPBMAdminPageFramework_WPUtility;
	...
	protected function _enqueueSRC($aEnqueueItem)
	{
		...
		if ($aEnqueueItem['translation']) wp_localize_script($aEnqueueItem['handle_id'], $aEnqueueItem['handle_id'], $aEnqueueItem['translation']);
	}

	public function _replyToEnqueueStyles()
	{
		foreach ($this->oProp->aEnqueuingStyles as $sKey => $aEnqueuingStyle) $this->_enqueueSRCByConditoin($aEnqueuingStyle);
	}

	public function _replyToEnqueueScripts()
	{
		foreach ($this->oProp->aEnqueuingScripts as $sKey => $aEnqueuingScript) $this->_enqueueSRCByConditoin($aEnqueuingScript);
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_HeadTag_MetaBox')) : class WPBMAdminPageFramework_HeadTag_MetaBox extends WPBMAdminPageFramework_HeadTag_Base
{
	...
	protected function _printClassSpecificStyles($sIDPrefix)
	{
		...
		if ($sStyle) echo "<style type='text/css' id='{$sIDPrefix}-{$this->oProp->sClassName}'>{$sStyle}</style>";
		...
		if ($sStyleIE) echo "<!--[if IE]><style type='text/css' id='{$sIDPrefix}-ie-{$this->oProp->sClassName}'>{$sStyleIE}</style><![endif]-->";
	}
	...
	protected function _printCommonStyles($sIDPrefix, $sClassName)
	{
		...
		$oCaller = $this->oProp->_getCallerObject();
		$sStyle = $this->oUtil->addAndApplyFilters($oCaller, "style_common_{$this->oProp->sClassName}", WPBMAdminPageFramework_Property_Base::$_sDefaultStyle);
		$sStyle = $this->oUtil->minifyCSS($sStyle);
		if ($sStyle) echo "<style type='text/css' id='{$sIDPrefix}'>{$sStyle}</style>";
		$sStyleIE = $this->oUtil->addAndApplyFilters($oCaller, "style_ie_common_{$this->oProp->sClassName}", WPBMAdminPageFramework_Property_Base::$_sDefaultStyleIE);
		$sStyleIE = $this->oUtil->minifyCSS($sStyleIE);
		if ($sStyleIE) echo "<!--[if IE]><style type='text/css' id='{$sIDPrefix}-ie'>{$sStyleIE}</style><![endif]-->";
	}
	...
	protected function _printClassSpecificScripts($sIDPrefix)
	{
		...
		if ($sScript) echo "<script type='text/javascript' id='{$sIDPrefix}-{$this->oProp->sClassName}'>{$sScript}</script>";
	}
	...
	protected function _printCommonScripts($sIDPrefix, $sClassName)
	{
		...
		$sScript = $this->oUtil->addAndApplyFilters($this->oProp->_getCallerObject(), "script_common_{$this->oProp->sClassName}", WPBMAdminPageFramework_Property_Base::$_sDefaultScript);
		if ($sScript) echo "<script type='text/javascript' id='{$sIDPrefix}'>{$sScript}</script>";
	}

	public function _enqueueStyles($aSRCs, $aPostTypes = array(), $aCustomArgs = array())
	{
		$aHandleIDs = array();
		foreach (( array )$aSRCs as $sSRC) $aHandleIDs[] = $this->_enqueueStyle($sSRC, $aPostTypes, $aCustomArgs);
		return $aHandleIDs;
	}
	...
	{
		$aHandleIDs = array();
		foreach (( array )$aSRCs as $sSRC) $aHandleIDs[] = $this->_enqueueScript($sSRC, $aPostTypes, $aCustomArgs);
		return $aHandleIDs;
	}
	...
	protected function _enqueueSRCByConditoin($aEnqueueItem)
	{
		...
		if (in_array($sCurrentPostType, $aEnqueueItem['aPostTypes'])) return $this->_enqueueSRC($aEnqueueItem);
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_HeadTag_Page')) : class WPBMAdminPageFramework_HeadTag_Page extends WPBMAdminPageFramework_HeadTag_Base
{
	public function _replyToAddStyle()
	{
		...
		$oCaller = $this->oProp->_getCallerObject();
		$sStyle = $this->oUtil->addAndApplyFilters($oCaller, $this->oUtil->getFilterArrayByPrefix('style_common_', $this->oProp->sClassName, $sPageSlug, $sTabSlug, false), WPBMAdminPageFramework_Property_Page::$_sDefaultStyle) . $this->oUtil->addAndApplyFilters($oCaller, $this->oUtil->getFilterArrayByPrefix('style_', $this->oProp->sClassName, $sPageSlug, $sTabSlug, false), $this->oProp->sStyle);
		$sStyle = $this->oUtil->minifyCSS($sStyle);
		if ($sStyle) echo "<style type='text/css' id='admin-page-framework-style_{$this->oProp->sClassName}'>{$sStyle}</style>";
		$sStyleIE = $this->oUtil->addAndApplyFilters($oCaller, $this->oUtil->getFilterArrayByPrefix('style_common_ie_', $this->oProp->sClassName, $sPageSlug, $sTabSlug, false), WPBMAdminPageFramework_Property_Page::$_sDefaultStyleIE) . $this->oUtil->addAndApplyFilters($oCaller, $this->oUtil->getFilterArrayByPrefix('style_ie_', $this->oProp->sClassName, $sPageSlug, $sTabSlug, false), $this->oProp->sStyleIE);
		$sStyleIE = $this->oUtil->minifyCSS($sStyleIE);
		if ($sStyleIE) echo "<!--[if IE]><style type='text/css' id='admin-page-framework-style-for-IE_{$this->oProp->sClassName}'>{$sStyleIE}</style><![endif]-->";
		$this->oProp->_bAddedStyle = true;
	}
	...
	public function _replyToAddScript()
	{
		...
		if (!$this->oProp->isPageAdded($sPageSlug)) return;
		$oCaller = $this->oProp->_getCallerObject();
		echo "<script type='text/javascript' id='admin-page-framework-script_{$this->oProp->sClassName}'>" . ($sScript = $this->oUtil->addAndApplyFilters($oCaller, $this->oUtil->getFilterArrayByPrefix('script_', $this->oProp->sClassName, $sPageSlug, $sTabSlug, false), $this->oProp->sScript)) . "</script>";
		$this->oProp->_bAddedScript = true;
	}
	...
	{
		$aHandleIDs = array();
		foreach (( array )$aSRCs as $sSRC) $aHandleIDs[] = $this->_enqueueStyle($sSRC, $sPageSlug, $sTabSlug, $aCustomArgs);
		return $aHandleIDs;
	}
	...
	{
		$aHandleIDs = array();
		foreach (( array )$aSRCs as $sSRC) $aHandleIDs[] = $this->_enqueueScript($sSRC, $sPageSlug, $sTabSlug, $aCustomArgs);
		return $aHandleIDs;
	}
	...
	protected function _enqueueSRCByConditoin($aEnqueueItem)
	{
		...
		if (!$sPageSlug && $this->oProp->isPageAdded($sCurrentPageSlug)) return $this->_enqueueSRC($aEnqueueItem);
		if (($sPageSlug && $sCurrentPageSlug == $sPageSlug) && ($sTabSlug && $sCurrentTabSlug == $sTabSlug)) return $this->_enqueueSRC($aEnqueueItem);
		if (($sPageSlug && !$sTabSlug) && ($sCurrentPageSlug == $sPageSlug)) return $this->_enqueueSRC($aEnqueueItem);
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_HeadTag_MetaBox_Page')) : class WPBMAdminPageFramework_HeadTag_MetaBox_Page extends WPBMAdminPageFramework_HeadTag_Page
{
	private function _isMetaBoxPage()
	{
		if (!isset($_GET['page'])) return false;
		if (in_array($_GET['page'], $this->oProp->aPageSlugs)) return true;
		return false;
	}

	public function _replyToAddStyle()
	{
		if (!$this->_isMetaBoxPage()) return;
		$this->_printCommonStyles('admin-page-framework-style-meta-box-common', get_class());
		$this->_printClassSpecificStyles('admin-page-framework-style-meta-box');
		$this->oProp->_bAddedStyle = true;
	}
...
if (!class_exists('WPBMAdminPageFramework_HeadTag_PostType')) : class WPBMAdminPageFramework_HeadTag_PostType extends WPBMAdminPageFramework_HeadTag_MetaBox
{
	public function _replyToAddStyle()
	{
		if (!(in_array($GLOBALS['pagenow'], array('edit.php', 'edit-tags.php')) && (isset($_GET['post_type']) && $_GET['post_type'] == $this->oProp->sPostType) || $this->oUtil->isPostDefinitionPage($this->oProp->sPostType))) return;
		if (isset($_GET['page']) && $_GET['page']) return;
		...
		$oCaller = $this->oProp->_getCallerObject();
		$sStyle = $this->oUtil->addAndApplyFilters($oCaller, "style_common_{$this->oProp->sClassName}", WPBMAdminPageFramework_Property_PostType::$_sDefaultStyle) . $this->oUtil->addAndApplyFilters($oCaller, "style_{$this->oProp->sClassName}", $this->oProp->sStyle);
		$sStyle = $this->oUtil->minifyCSS($sStyle);
		if ($sStyle) echo "<style type='text/css' id='admin-page-framework-style-post-type'>{$sStyle}</style>";
		$sStyleIE = $this->oUtil->addAndApplyFilters($oCaller, "style_ie_{$this->oProp->sClassName}", WPBMAdminPageFramework_Property_PostType::$_sDefaultStyleIE) . $this->oUtil->addAndApplyFilters($oCaller, "style_ie_{$this->oProp->sClassName}", $this->oProp->sStyleIE);
		$sStyleIE = $this->oUtil->minifyCSS($sStyleIE);
		if ($sStyleIE) echo "<!--[if IE]><style type='text/css' id='admin-page-framework-style-post-type'>{$sStyleIE}</style><![endif]-->";
	}
	...
	public function _replyToAddScript()
	{
		...
		if ($sScript) echo "<script type='text/javascript' id='admin-page-framework-script-post-type'>{$sScript}</script>";
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_HeadTag_TaxonomyField')) : class WPBMAdminPageFramework_HeadTag_TaxonomyField extends WPBMAdminPageFramework_HeadTag_MetaBox
{
	public function _replyToAddStyle()
	{
		if ($GLOBALS['pagenow'] != 'edit-tags.php') return;
		$this->_printCommonStyles('admin-page-framework-style-taxonomy-field-common', get_class());
	...
	{
		if ($GLOBALS['pagenow'] != 'edit-tags.php') return;
		$this->_printCommonScripts('admin-page-framework-style-taxonomy-field-common', get_class());
...
if (!class_exists('WPBMAdminPageFramework_MetaBox')) : abstract class WPBMAdminPageFramework_MetaBox_Base
{
	protected $oDebug;
	protected $oUtil;
	protected $oMsg;
	...
	function __construct($sMetaBoxID, $sTitle, $asPostTypeOrScreenID = array('post'), $sContext = 'normal', $sPriority = 'default', $sCapability = 'edit_posts', $sTextDomain = 'admin-page-framework')
	{
		...
		$this->oUtil = new WPBMAdminPageFramework_WPUtility;
		$this->oMsg = WPBMAdminPageFramework_Message::instantiate($sTextDomain);
		$this->oDebug = new WPBMAdminPageFramework_Debug;
		$this->oProp = isset($this->oProp) ? $this->oProp : new WPBMAdminPageFramework_Property_MetaBox($this, get_class($this), $sCapability);
		...
		if ($this->oProp->bIsAdmin) {
			add_action('wp_loaded', array($this, '_replyToLoadDefaultFieldTypeDefinitions'), 10);
			add_action('wp_loaded', array($this, 'setUp'), 11);
			add_action('current_screen', array($this, '_replyToRegisterFormElements'));
	...
	public function enqueueStyles($aSRCs, $_vArg2 = null, $_vArg3 = null)
	{
	}
	...
	{
		new WPBMAdminPageFramework_FieldTypeRegistration($this->oProp->aFieldTypeDefinitions, $this->oProp->sClassName, $this->oMsg);
	...
	public function _replyToPrintMetaBoxContents($oPost, $vArgs)
	{
		...
		if (isset($this->oProp->aOptions)) $this->setOptionArray(isset($oPost->ID) ? $oPost->ID : (isset($_GET['page']) ? $_GET['page'] : null), $this->oForm->aConditionedFields);
		$this->oForm->setDynamicElements($this->oProp->aOptions);
		$oFieldsTable = new WPBMAdminPageFramework_FormTable($this->oProp->aFieldTypeDefinitions, $this->oMsg);
		...
		if (is_numeric($isPostIDOrPageSlug) && is_int($isPostIDOrPageSlug + 0)) : $_iPostID = $isPostIDOrPageSlug;
			foreach ($aFields as $_sSectionID => $_aFields) {
				if ($_sSectionID == '_default') {
					foreach ($_aFields as $_aField) $this->oProp->aOptions[$_aField['field_id']] = get_post_meta($_iPostID, $_aField['field_id'], true);
				}
			...
			foreach ($_aFields as $_iSubSectionIndexOrFieldID => $_aSubSectionOrField) {
				if (is_numeric($_iSubSectionIndexOrFieldID) && is_int($_iSubSectionIndexOrFieldID + 0)) {
					...
					foreach ($_aSubSectionOrField as $_aField) {
						WPBMAdminPageFramework_FieldTypeRegistration::_setFieldHeadTagElements($_aField, $this->oProp, $this->oHeadTag);
						if ($_aField['help']) $this->oHelpPane->_addHelpTextForFormFields($_aField['title'], $_aField['help'], $_aField['help_aside']);
					}
					continue;
				}
	...
	public function _replyToGetFieldOutput($aField)
	{
		...
		$oField = new WPBMAdminPageFramework_FormField($aField, $this->oProp->aOptions, array(), $this->oProp->aFieldTypeDefinitions, $this->oMsg);
		$sFieldOutput = $oField->_getFieldOutput();
		unset($oField);
		return $this->oUtil->addAndApplyFilters($this, array('field_' . $this->oProp->sClassName . '_' . $aField['field_id']), $sFieldOutput, $aField);
	}
	...
	public function _replyToSaveMetaBoxFields($iPostID)
	{
		...
		$aInput = $this->getInputArray($this->oForm->aFields, $this->oForm->aSections);
		...
		foreach ($aInput as $_sSectionOrFieldID => $_vValue) {
			...
			if ($_vValue == $vSavedValue) continue;
	...
	{
		$aSavedMeta = array();
		foreach ($aInputStructure as $_sSectionORFieldID => $_v) $aSavedMeta[$_sSectionORFieldID] = get_post_meta($iPostID, $_sSectionORFieldID, true);
		return $aSavedMeta;
	}
...
if (!class_exists('WPBMAdminPageFramework_MetaBox')) : abstract class WPBMAdminPageFramework_MetaBox extends WPBMAdminPageFramework_MetaBox_Base
{
	static protected $_sFieldsType = 'post_meta_box';

	function __construct($sMetaBoxID, $sTitle, $asPostTypeOrScreenID = array('post'), $sContext = 'normal', $sPriority = 'default', $sCapability = 'edit_posts', $sTextDomain = 'admin-page-framework')
	{
		$this->oProp = new WPBMAdminPageFramework_Property_MetaBox($this, get_class($this), $sCapability);
		parent::__construct($sMetaBoxID, $sTitle, $asPostTypeOrScreenID, $sContext, $sPriority, $sCapability, $sTextDomain);
		$this->oHeadTag = new WPBMAdminPageFramework_HeadTag_MetaBox($this->oProp);
		$this->oHelpPane = new WPBMAdminPageFramework_HelpPane_MetaBox($this->oProp);
		...
		$this->oForm = new WPBMAdminPageFramework_FormElement($this->oProp->sFieldsType, $sCapability);
	...
	public function _replyToAddMetaBox()
	{
		foreach ($this->oProp->aPostTypes as $sPostType) add_meta_box($this->oProp->sMetaBoxID, $this->oProp->sTitle, array($this, '_replyToPrintMetaBoxContents'), $sPostType, $this->oProp->sContext, $this->oProp->sPriority, null);
	}
	...
	public function _replyToRegisterFormElements()
	{
		...
		$this->oForm->format();
		$this->oForm->applyConditions();
		if (isset($this->oProp->aOptions)) $this->setOptionArray(isset($GLOBALS['post']->ID) ? $GLOBALS['post']->ID : (isset($_GET['page']) ? $_GET['page'] : null), $this->oForm->aConditionedFields);
		$this->oForm->setDynamicElements($this->oProp->aOptions);
		$this->_registerFields($this->oForm->aConditionedFields);
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_MetaBox_Page')) : abstract class WPBMAdminPageFramework_MetaBox_Page extends WPBMAdminPageFramework_MetaBox_Base
{
	...
	function __construct($sMetaBoxID, $sTitle, $asPageSlugs = array(), $sContext = 'normal', $sPriority = 'default', $sCapability = 'manage_options', $sTextDomain = 'admin-page-framework')
	{
		...
		$this->oProp = new WPBMAdminPageFramework_Property_MetaBox_Page($this, get_class($this), $sCapability);
		parent::__construct($sMetaBoxID, $sTitle, $asPageSlugs, $sContext, $sPriority, $sCapability, $sTextDomain);
		$this->oHeadTag = new WPBMAdminPageFramework_HeadTag_MetaBox_Page($this->oProp);
		$this->oHelpPane = new WPBMAdminPageFramework_HelpPane_MetaBox($this->oProp);
		...
		foreach ($this->oProp->aPageSlugs as $sIndexOrPageSlug => $asTabArrayOrPageSlug) {
			...
			add_filter("validation_{$_sPageSlug}", array($this, '_replyToValidateOptions'), 10, 2);
			foreach ($_aTabs as $sTabSlug) add_filter("validation_saved_options_{$_sPageSlug}_{$_sTabSlug}", array($this, '_replyToFilterPageOptions'));
		}
		$this->oUtil->addAndDoAction($this, "start_{$this->oProp->sClassName}");
	}
	...
	private function _isMetaBoxPage($sPageSlug)
	{
		...
		if (in_array($sPageSlug, $this->oProp->aPageSlugs)) return true;
		return false;
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_TaxonomyField')) : abstract class WPBMAdminPageFramework_TaxonomyField extends WPBMAdminPageFramework_MetaBox_Base
{
	protected $oProp;
	protected $oHeadTag;
	protected $oHelpPane;
	...
	function __construct($asTaxonomySlug, $sOptionKey = '', $sCapability = 'manage_options', $sTextDomain = 'admin-page-framework')
	{
		...
		$this->oProp = new WPBMAdminPageFramework_Property_TaxonomyField($this, get_class($this), $sCapability);
		$this->oUtil = new WPBMAdminPageFramework_WPUtility;
		$this->oMsg = WPBMAdminPageFramework_Message::instantiate($sTextDomain);
		$this->oDebug = new WPBMAdminPageFramework_Debug;
		$this->oHeadTag = new WPBMAdminPageFramework_HeadTag_TaxonomyField($this->oProp);
		$this->oHelpPane = new WPBMAdminPageFramework_HelpPane_TaxonomyField($this->oProp);
	...
	private function _getFieldsOutput($iTermID, $bRenderTableRow)
	{
		...
		$this->oForm->format();
		$oFieldsTable = new WPBMAdminPageFramework_FormTable($this->oProp->aFieldTypeDefinitions, $this->oMsg);
		$aOutput[] = $bRenderTableRow ? $oFieldsTable->getFieldRows($this->oForm->aFields['_default'], array($this, '_replyToGetFieldOutput')) : $oFieldsTable->getFields($this->oForm->aFields['_default'], array($this, '_replyToGetFieldOutput'));
	...
	function __call($sMethodName, $aArgs = null)
	{
		if (isset($_GET['taxonomy']) && $_GET['taxonomy']) : if (substr($sMethodName, 0, strlen('columns_' . $_GET['taxonomy'])) == 'columns_' . $_GET['taxonomy']) return $aArgs[0];
			if (substr($sMethodName, 0, strlen('sortable_columns_' . $_GET['taxonomy'])) == 'sortable_columns_' . $_GET['taxonomy']) return $aArgs[0];
			if (substr($sMethodName, 0, strlen('cell_' . $_GET['taxonomy'])) == 'cell_' . $_GET['taxonomy']) return $aArgs[0]; endif;
...
if (!class_exists('WPBMAdminPageFramework_Base')) : abstract class WPBMAdminPageFramework_Base
{
	...
	{
		$this->oProp = new WPBMAdminPageFramework_Property_Page($this, $sCallerPath, get_class($this), $sOptionKey, $sCapability);
		$this->oMsg = WPBMAdminPageFramework_Message::instantiate($sTextDomain);
		$this->oPageLoadInfo = WPBMAdminPageFramework_PageLoadInfo_Page::instantiate($this->oProp, $this->oMsg);
		$this->oHelpPane = new WPBMAdminPageFramework_HelpPane_Page($this->oProp);
		$this->oLink = new WPBMAdminPageFramework_Link_Page($this->oProp, $this->oMsg);
		$this->oHeadTag = new WPBMAdminPageFramework_HeadTag_Page($this->oProp);
		$this->oUtil = new WPBMAdminPageFramework_WPUtility;
		$this->oDebug = new WPBMAdminPageFramework_Debug;
		if ($this->oProp->bIsAdmin) add_action('wp_loaded', array($this, 'setUp'));
	}
	...
	public function __call($sMethodName, $aArgs = null)
	{
		...
		if (substr($sMethodName, 0, strlen('validation_pre_')) == 'validation_pre_') return $this->_doValidationCall($sMethodName, $aArgs[0]);
		if (substr($sMethodName, 0, strlen('load_pre_')) == 'load_pre_') return $this->_doPageLoadCall(substr($sMethodName, strlen('load_pre_')), $sTabSlug, $aArgs[0]);
		if ($sMethodName == $this->oProp->sClassHash . '_page_' . $sPageSlug) return $this->_renderPage($sPageSlug, $sTabSlug);
		if ($this->_isFrameworkCallbackMethod($sMethodName)) return isset($aArgs[0]) ? $aArgs[0] : null;
	}

	private function _isFrameworkCallbackMethod($sMethodName)
	{
		foreach (self::$_aHookPrefixes as $sPrefix) if (substr($sMethodName, 0, strlen($sPrefix)) == $sPrefix) return true;
		return false;
	}
...
if (!class_exists('WPBMAdminPageFramework_Page_MetaBox')) : abstract class WPBMAdminPageFramework_Page_MetaBox extends WPBMAdminPageFramework_Base
{
	function __construct($sOptionKey = null, $sCallerPath = null, $sCapability = null, $sTextDomain = 'admin-page-framework')
	{
	...
	{
		if (!isset($GLOBALS['wp_meta_boxes'][$GLOBALS['page_hook']][$sContext]) || count($GLOBALS['wp_meta_boxes'][$GLOBALS['page_hook']][$sContext]) <= 0) return;
	...
	{
		if (isset($GLOBALS['wp_meta_boxes'][$GLOBALS['page_hook']]['side']) && count($GLOBALS['wp_meta_boxes'][$GLOBALS['page_hook']]['side']) > 0) return 2;
		return 1;
	...
	public function _replyToSetNumberOfScreenLayoutColumns($aColumns, $sScreenID)
	{
		...
		add_filter('get_user_option_' . 'screen_layout_' . $GLOBALS['page_hook'], array($this, '_replyToReturnDefaultNumberOfScreenColumns'), 10, 3);
		if ($sScreenID == $GLOBALS['page_hook']) $aColumns[$GLOBALS['page_hook']] = 2;
		return $aColumns;
	}

	private function _isMetaBoxAdded($sPageSlug = '')
	{
		if (!isset($GLOBALS['aWPBMAdminPageFramework']['aMetaBoxForPagesClasses'])) return false;
		if (!is_array($GLOBALS['aWPBMAdminPageFramework']['aMetaBoxForPagesClasses'])) return false;
		$sPageSlug = $sPageSlug ? $sPageSlug : (isset($_GET['page']) ? $_GET['page'] : '');
		if (!$sPageSlug) return false;
		foreach ($GLOBALS['aWPBMAdminPageFramework']['aMetaBoxForPagesClasses'] as $sClassName => $oMetaBox) if ($this->_isPageOfMetaBox($sPageSlug, $oMetaBox)) return true;
		return false;
	}
	...
	private function _isPageOfMetaBox($sPageSlug, $oMetaBox)
	{
		...
		if (!array_key_exists($sPageSlug, $oMetaBox->oProp->aPageSlugs)) return false;
	...
	{
		if ($sOptionKey != 'screen_layout_' . $GLOBALS['page_hook']) return $vStoredData;
	...
	{
		if (isset($GLOBALS['aWPBMAdminPageFramework']['bAddedMetaBoxScript'])) return;
		$GLOBALS['aWPBMAdminPageFramework']['bAddedMetaBoxScript'] = true; ?>
...
if (!class_exists('WPBMAdminPageFramework_Page')) : abstract class WPBMAdminPageFramework_Page extends WPBMAdminPageFramework_Page_MetaBox
{
	...
	private function _printMainContent($sPageSlug, $sTabSlug)
	{
		...
		if ($_bIsSideMetaboxExist) echo "<div id='post-body-content'>";
		ob_start();
	...
	{
		return $this->oProp->bEnableForm ? "<form action='options.php' method='post' enctype='{$this->oProp->sFormEncType}' id='admin-page-framework-form'>" : "";
	}

	private function _getFormClosingTag($sPageSlug, $sTabSlug)
	{
		return $this->oProp->bEnableForm ? "<input type='hidden' name='page_slug' value='{$sPageSlug}' />" . PHP_EOL . "<input type='hidden' name='tab_slug' value='{$sTabSlug}' />" . PHP_EOL . "</form><!-- End Form -->" : '';
	}
	...
	private function _showSettingsErrors()
	{
		...
		if (count($aSettingsMessages) > 1) $this->_removeDefaultSettingsNotice();
		settings_errors($this->oProp->sOptionKey);
	}

	private function _removeDefaultSettingsNotice()
	{
		global $wp_settings_errors;
		$aDefaultMessages = array($this->oMsg->__('option_cleared'), $this->oMsg->__('option_updated'),);
		foreach (( array )$wp_settings_errors as $iIndex => $aDetails) {
			if ($aDetails['setting'] != $this->oProp->sOptionKey) continue;
			if (in_array($aDetails['message'], $aDefaultMessages)) unset($wp_settings_errors[$iIndex]);
		}
	}
	...
	private function _getInPageTabs($sCurrentPageSlug, $sTag = 'h3', $aOutput = array())
	{
		...
		if (!$this->oProp->aPages[$sCurrentPageSlug]['show_in_page_tabs']) return isset($this->oProp->aInPageTabs[$sCurrentPageSlug][$sCurrentTabSlug]['title']) ? "<{$sTag}>{$this->oProp->aInPageTabs[$sCurrentPageSlug][$sCurrentTabSlug]['title']}</{$sTag}>" : "";
	...
	public function _replyToFinalizeInPageTabs()
	{
		foreach ($this->oProp->aPages as $sPageSlug => $aPage) {
			...
			foreach ($this->oProp->aInPageTabs[$sPageSlug] as &$aInPageTab) $aInPageTab = $aInPageTab + self::$_aStructure_InPageTabElements;
...
if (!class_exists('WPBMAdminPageFramework_Menu')) : abstract class WPBMAdminPageFramework_Menu extends WPBMAdminPageFramework_Page
{
	protected static $_aBuiltInRootMenuSlugs = array('dashboard' => 'index.php', 'posts' => 'edit.php', 'media' => 'upload.php', 'links' => 'link-manager.php', 'pages' => 'edit.php?post_type=page', 'comments' => 'edit-comments.php', 'appearance' => 'themes.php', 'plugins' => 'plugins.php', 'users' => 'users.php', 'tools' => 'tools.php', 'settings' => 'options-general.php', 'network admin' => "network_admin_menu",);
	...
	public function setRootMenuPage($sRootMenuLabel, $sIcon16x16 = null, $iMenuPosition = null)
	{
		...
		$sSlug = $this->_isBuiltInMenuItem($sRootMenuLabel);
		$this->oProp->aRootMenu = array('sTitle' => $sRootMenuLabel, 'sPageSlug' => $sSlug ? $sSlug : $this->oProp->sClassName, 'sIcon16x16' => $this->oUtil->resolveSRC($sIcon16x16), 'iPosition' => $iMenuPosition, 'fCreateRoot' => $sSlug ? false : true,);
	}

	private function _isBuiltInMenuItem($sMenuLabel)
	{
		$sMenuLabelLower = strtolower($sMenuLabel);
		if (array_key_exists($sMenuLabelLower, self::$_aBuiltInRootMenuSlugs)) return self::$_aBuiltInRootMenuSlugs[$sMenuLabelLower];
	}
	...
	public function addSubMenuItem(array $aSubMenuItem)
	{
		if (isset($aSubMenuItem['href'])) $this->addSubMenuLink($aSubMenuItem); else $this->addSubMenuPage($aSubMenuItem);
	}
	...
	public function _replyToBuildMenu()
	{
		...
		if ($this->oProp->aRootMenu['fCreateRoot']) remove_submenu_page($this->oProp->aRootMenu['sPageSlug'], $this->oProp->aRootMenu['sPageSlug']);
	}
...
if (!class_exists('WPBMAdminPageFramework_Setting_Base')) : abstract class WPBMAdminPageFramework_Setting_Base extends WPBMAdminPageFramework_Menu
{
	...
	function __construct($sOptionKey = null, $sCallerPath = null, $sCapability = null, $sTextDomain = 'admin-page-framework')
	{
		...
		parent::__construct($sOptionKey, $sCallerPath, $sCapability, $sTextDomain);
		$this->oProp->sFieldsType = self::$_sFieldsType;
		$this->oForm = new WPBMAdminPageFramework_FormElement_Page($this->oProp->sFieldsType, $this->oProp->sCapability);
	}
	...
	private function _getFieldErrors($sPageSlug, $bDelete = true)
	{
		...
		if ($bDelete) delete_transient($sTransient);
		return $aFieldErrors;
	}
	...
	public function _replyToRegisterSettings()
	{
		...
		foreach ($this->oForm->aConditionedSections as $_aSection) {
			add_settings_section($_aSection['section_id'], "<a id='{$_aSection['section_id']}'></a>" . $_aSection['title'], array($this, 'section_pre_' . $_aSection['section_id']), $_aSection['page_slug']);
			...
			foreach ($_aSubSectionOrFields as $_sSubSectionIndexOrFieldID => $_aSubSectionOrField) {
				if (is_numeric($_sSubSectionIndexOrFieldID) && is_int($_sSubSectionIndexOrFieldID + 0)) {
					...
					foreach ($_aSubSection as $__sFieldID => $__aField) {
						add_settings_field($__aField['section_id'] . '_' . $_iSubSectionIndex . '_' . $__aField['field_id'], "<a id='{$__aField['section_id']}_{$_iSubSectionIndex}_{$__aField['field_id']}'></a><span title='{$__aField['tip']}'>{$__aField['title']}</span>", null, $this->oForm->getPageSlugBySectionID($__aField['section_id']), $__aField['section_id']);
						WPBMAdminPageFramework_FieldTypeRegistration::_setFieldHeadTagElements($__aField, $this->oProp, $this->oHeadTag);
					}
					continue;
				}
				$aField = $_aSubSectionOrField;
				add_settings_field($aField['section_id'] . '_' . $aField['field_id'], "<a id='{$aField['section_id']}_{$aField['field_id']}'></a><span title='{$aField['tip']}'>{$aField['title']}</span>", null, $this->oForm->getPageSlugBySectionID($aField['section_id']), $aField['section_id']);
				WPBMAdminPageFramework_FieldTypeRegistration::_setFieldHeadTagElements($aField, $this->oProp, $this->oHeadTag);
		...
		register_setting($this->oProp->sOptionKey, $this->oProp->sOptionKey, array($this, 'validation_pre_' . $this->oProp->sClassName));
	}
	...
	public function _replyToGetFieldOutput($aField)
	{
		...
		$sFieldType = isset($this->oProp->aFieldTypeDefinitions[$aField['type']]['hfRenderField']) && is_callable($this->oProp->aFieldTypeDefinitions[$aField['type']]['hfRenderField']) ? $aField['type'] : 'default';
		$oField = new WPBMAdminPageFramework_FormField($aField, $this->oProp->aOptions, $this->aFieldErrors, $this->oProp->aFieldTypeDefinitions, $this->oMsg);
		$sFieldOutput = $oField->_getFieldOutput();
		unset($oField);
		return $this->oUtil->addAndApplyFilters($this, array(isset($aField['section_id']) && $aField['section_id'] != '_default' ? 'field_' . $this->oProp->sClassName . '_' . $aField['section_id'] . '_' . $_sFieldID : 'field_' . $this->oProp->sClassName . '_' . $_sFieldID,), $sFieldOutput, $aField);
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_Setting')) : abstract class WPBMAdminPageFramework_Setting extends WPBMAdminPageFramework_Setting_Base
{
	public function setSettingNotice($sMsg, $sType = 'error', $sID = null, $bOverride = true)
	{
		$aWPSettingsErrors = isset($GLOBALS['wp_settings_errors']) ? ( array )$GLOBALS['wp_settings_errors'] : array();
		$sID = isset($sID) ? $sID : $this->oProp->sOptionKey;
		foreach ($aWPSettingsErrors as $iIndex => $aSettingsError) {
			...
			if ($aSettingsError['code'] === $sID) {
				if (!$bOverride) return; else unset($aWPSettingsErrors[$iIndex]);
			}
		}
		add_settings_error($this->oProp->sOptionKey, $sID, $sMsg, $sType);
	}
	...
	public function setFieldErrors($aErrors, $sID = null, $nSavingDuration = 300)
	{
		...
		set_transient(md5($this->oProp->sClassName . '_' . $sID), $aErrors, $nSavingDuration);
	}
...
if (!class_exists('WPBMAdminPageFramework_Setting_Port')) : abstract class WPBMAdminPageFramework_Setting_Port extends WPBMAdminPageFramework_Setting
{
	protected function _importOptions($aStoredOptions, $sPageSlug, $sTabSlug)
	{
		$oImport = new WPBMAdminPageFramework_ImportOptions($_FILES['__import'], $_POST['__import']);
		...
		$bEmpty = empty($vData);
		$this->setSettingNotice($bEmpty ? $this->oMsg->__('not_imported_data') : $this->oMsg->__('imported_data'), $bEmpty ? 'error' : 'updated', $this->oProp->sOptionKey, false);
	...
	{
		$oExport = new WPBMAdminPageFramework_ExportOptions($_POST['__export'], $this->oProp->sClassName);
...
if (!class_exists('WPBMAdminPageFramework_Setting_Validation')) : abstract class WPBMAdminPageFramework_Setting_Validation extends WPBMAdminPageFramework_Setting_Port
{
	protected function _doValidationCall($sMethodName, $aInput)
	{
		$sTabSlug = isset($_POST['tab_slug']) ? $_POST['tab_slug'] : '';
		...
		$bIsReset = isset($_POST['__submit']) ? $this->_getPressedSubmitButtonData($_POST['__submit'], 'is_reset') : '';
		$sKeyToReset = isset($_POST['__submit']) ? $this->_getPressedSubmitButtonData($_POST['__submit'], 'reset_key') : '';
		$sSubmitSectionID = isset($_POST['__submit']) ? $this->_getPressedSubmitButtonData($_POST['__submit'], 'section_id') : '';
		$this->oUtil->addAndDoActions($this, array("submit_{$this->oProp->sClassName}_{$sPressedInputID}", $sSubmitSectionID ? "submit_{$this->oProp->sClassName}_{$sSubmitSectionID}_{$sPressedFieldID}" : "submit_{$this->oProp->sClassName}_{$sPressedFieldID}", $sSubmitSectionID ? "submit_{$this->oProp->sClassName}_{$sSubmitSectionID}" : null, isset($_POST['tab_slug']) ? "submit_{$this->oProp->sClassName}_{$sPageSlug}_{$sTabSlug}" : null, "submit_{$this->oProp->sClassName}_{sPageSlug}", "submit_{$this->oProp->sClassName}",));
		if (isset($_POST['__import']['submit'], $_FILES['__import'])) return $this->_importOptions($this->oProp->aOptions, $sPageSlug, $sTabSlug);
		if (isset($_POST['__export']['submit'])) die($this->_exportOptions($this->oProp->aOptions, $sPageSlug, $sTabSlug));
		if ($bIsReset) return $this->_askResetOptions($sPressedInputName, $sPageSlug, $sSubmitSectionID);
		if (isset($_POST['__submit']) && $sLinkURL = $this->_getPressedSubmitButtonData($_POST['__submit'], 'link_url')) die(wp_redirect($sLinkURL));
		if (isset($_POST['__submit']) && $sRedirectURL = $this->_getPressedSubmitButtonData($_POST['__submit'], 'redirect_url')) $this->_setRedirectTransients($sRedirectURL);
		$aInput = $this->_getFilteredOptions($aInput, $sPageSlug, $sTabSlug);
		if ($sKeyToReset) $aInput = $this->_resetOptions($sKeyToReset, $aInput);
		$bEmpty = empty($aInput);
		$this->setSettingNotice($bEmpty ? $this->oMsg->__('option_cleared') : $this->oMsg->__('option_updated'), $bEmpty ? 'error' : 'updated', $this->oProp->sOptionKey, false);
		return $aInput;
	}
	...
	private function _askResetOptions($sPressedInputName, $sPageSlug, $sSectionID)
	{
		...
		$aErrors = array();
		if ($sSectionID) $aErrors[$sSectionID][$sFieldID] = $this->oMsg->__('reset_options'); else $aErrors[$sFieldID] = $this->oMsg->__('reset_options');
	...
	private function _getPressedSubmitButtonData($aPostElements, $sTargetKey = 'field_id')
	{
		foreach ($aPostElements as $sInputID => $aSubElements) {
			...
			if (count($aNameKeys) == 2 && isset($_POST[$aNameKeys[0]][$aNameKeys[1]])) return isset($aSubElements[$sTargetKey]) ? $aSubElements[$sTargetKey] : null;
			if (count($aNameKeys) == 3 && isset($_POST[$aNameKeys[0]][$aNameKeys[1]][$aNameKeys[2]])) return isset($aSubElements[$sTargetKey]) ? $aSubElements[$sTargetKey] : null;
			if (count($aNameKeys) == 4 && isset($_POST[$aNameKeys[0]][$aNameKeys[1]][$aNameKeys[2]][$aNameKeys[3]])) return isset($aSubElements[$sTargetKey]) ? $aSubElements[$sTargetKey] : null;
		}
		return null;
	}
	...
	private function _getFilteredOptions($aInput, $sPageSlug, $sTabSlug)
	{
		...
		foreach ($_aInput as $sID => $aSectionOrFields) {
			if ($this->oForm->isSection($sID)) foreach ($aSectionOrFields as $sFieldID => $aFields) $aInput[$sID][$sFieldID] = $this->oUtil->addAndApplyFilter($this, "validation_{$this->oProp->sClassName}_{$sID}_{$sFieldID}", $aInput[$sID][$sFieldID], isset($_aOptions[$sID][$sFieldID]) ? $_aOptions[$sID][$sFieldID] : null);
		...
		$_aPageOptions = $this->oForm->getPageOptions($_aOptions, $sPageSlug);
		...
		if ($sTabSlug && $sPageSlug) {
			$_aTabOnlyOptions = $this->oForm->getTabOnlyOptions($_aOptions, $sPageSlug, $sTabSlug);
			$_aTabOptions = $this->oForm->getTabOptions($_aOptions, $sPageSlug, $sTabSlug);
			...
			$aInput = $this->oUtil->uniteArrays($aInput, $this->oUtil->invertCastArrayContents($_aTabOptions, $_aTabOnlyOptions), $this->oForm->getOtherTabOptions($_aOptions, $sPageSlug, $sTabSlug));
		}
		if ($sPageSlug) {
			$aInput = $this->oUtil->addAndApplyFilter($this, "validation_{$sPageSlug}", $aInput, $_aPageOptions);
			$_aPageOptions = $sTabSlug && !empty($_aTabOptions) ? $this->oUtil->invertCastArrayContents($_aPageOptions, $_aTabOptions) : $_aPageOptions;
			$aInput = $this->oUtil->uniteArrays($aInput, $_aPageOptions, $this->oUtil->invertCastArrayContents($this->oForm->getOtherPageOptions($_aOptions, $sPageSlug), $_aPageOptions));
		}
	...
	private function _removePageElements($aOptions, $sPageSlug, $sTabSlug)
	{
		...
		if ($sTabSlug && $sPageSlug) return $this->oForm->getOtherTabOptions($aOptions, $sPageSlug, $sTabSlug);
		return $this->oForm->getOtherPageOptions($aOptions, $sPageSlug);
	}
} endif;
if (!class_exists('WPBMAdminPageFramework')) : abstract class WPBMAdminPageFramework extends WPBMAdminPageFramework_Setting_Validation
{
	public function __construct($sOptionKey = null, $sCallerPath = null, $sCapability = null, $sTextDomain = 'admin-page-framework')
	{
		parent::__construct($sOptionKey, $sCallerPath ? $sCallerPath : WPBMAdminPageFramework_Utility::getCallerScriptPath(__FILE__), $sCapability, $sTextDomain);
		$this->oUtil->addAndDoAction($this, 'start_' . $this->oProp->sClassName);
	}
	...
	{
		foreach ($this->oProp->aAdminNotices as $aAdminNotice) echo "<div class='{$aAdminNotice['sClassSelector']}' id='{$aAdminNotice['sID']}' ><p>" . $aAdminNotice['sMessage'] . "</p></div>";
	}
	...
	{
		return WPBMAdminPageFramework_WPUtility::getOption($sOptionKey, $asKey, $vDefault);
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_PostType')) : abstract class WPBMAdminPageFramework_PostType
{
	protected $oUtil;
	protected $oLink;
	...
	{
		$this->oUtil = new WPBMAdminPageFramework_WPUtility;
		$this->oProp = new WPBMAdminPageFramework_Property_PostType($this, $sCallerPath ? trim($sCallerPath) : WPBMAdminPageFramework_Utility::getCallerScriptPath(__FILE__), get_class($this));
		$this->oMsg = WPBMAdminPageFramework_Message::instantiate($sTextDomain);
		$this->oHeadTag = new WPBMAdminPageFramework_HeadTag_PostType($this->oProp);
		$this->oPageLoadInfo = WPBMAdminPageFramework_PageLoadInfo_PostType::instantiate($this->oProp, $this->oMsg);
		$this->oDebug = new WPBMAdminPageFramework_Debug;
		$this->oProp->sPostType = $this->oUtil->sanitizeSlug($sPostType);
		$this->oProp->aPostTypeArgs = $aArgs;
		$this->oProp->aColumnHeaders = array('cb' => '<input type="checkbox" />', 'title' => $this->oMsg->__('title'), 'author' => $this->oMsg->__('author'), 'comments' => '<div class="comment-grey-bubble"></div>', 'date' => $this->oMsg->__('date'),);
		...
		if ($this->oProp->sPostType != '' && $this->oProp->bIsAdmin) {
			...
			$this->oLink = new WPBMAdminPageFramework_Link_PostType($this->oProp, $this->oMsg);
	...
	protected function addTaxonomy($sTaxonomySlug, $aArgs)
	{
		...
		if (isset($aArgs['show_table_filter']) && $aArgs['show_table_filter']) $this->oProp->aTaxonomyTableFilters[] = $sTaxonomySlug;
		if (isset($aArgs['show_in_sidebar_menus']) && !$aArgs['show_in_sidebar_menus']) $this->oProp->aTaxonomyRemoveSubmenuPages["edit-tags.php?taxonomy={$sTaxonomySlug}&amp;post_type={$this->oProp->sPostType}"] = "edit.php?post_type={$this->oProp->sPostType}";
		if (count($this->oProp->aTaxonomyTableFilters) == 1) add_action('init', array($this, '_replyToRegisterTaxonomies'));
		if (count($this->oProp->aTaxonomyRemoveSubmenuPages) == 1) add_action('admin_menu', array($this, '_replyToRemoveTexonomySubmenuPages'), 999);
	}
	...
	public function _replyToAddStyle()
	{
		if (!isset($_GET['post_type']) || $_GET['post_type'] != $this->oProp->sPostType) return;
		if (isset($this->oProp->aPostTypeArgs['screen_icon']) && $this->oProp->aPostTypeArgs['screen_icon']) $this->oProp->sStyle .= $this->getStylesForPostTypeScreenIcon($this->oProp->aPostTypeArgs['screen_icon']);
		$this->oProp->sStyle = $this->oUtil->addAndApplyFilters($this, "style_{$this->oProp->sClassName}", $this->oProp->sStyle);
		if (!empty($this->oProp->sStyle)) echo "<style type='text/css' id='admin-page-framework-style-post-type'>" . $this->oProp->sStyle . "</style>";
	}
	...
	{
		foreach ($this->oProp->aTaxonomies as $sTaxonomySlug => $aArgs) register_taxonomy($sTaxonomySlug, $this->oProp->sPostType, $aArgs);
	}

	public function _replyToRemoveTexonomySubmenuPages()
	{
		foreach ($this->oProp->aTaxonomyRemoveSubmenuPages as $sSubmenuPageSlug => $sTopLevelPageSlug) remove_submenu_page($sTopLevelPageSlug, $sSubmenuPageSlug);
	}
	...
	public function _replyToAddAuthorTableFilter()
	{
		...
		if (!(isset($_GET['post_type']) && post_type_exists($_GET['post_type']) && in_array(strtolower($_GET['post_type']), array($this->oProp->sPostType)))) return;
	...
	public function _replyToAddTaxonomyTableFilter()
	{
		...
		if ($oPostCount->publish + $oPostCount->future + $oPostCount->draft + $oPostCount->pending + $oPostCount->private + $oPostCount->trash == 0) return;
		foreach (get_object_taxonomies($GLOBALS['typenow']) as $sTaxonomySulg) {
			...
			wp_dropdown_categories(array('show_option_all' => $this->oMsg->__('show_all') . ' ' . $oTaxonomy->label, 'taxonomy' => $sTaxonomySulg, 'name' => $oTaxonomy->name, 'orderby' => 'name', 'selected' => intval(isset($_GET[$sTaxonomySulg])), 'hierarchical' => $oTaxonomy->hierarchical, 'show_count' => true, 'hide_empty' => false, 'hide_if_empty' => false, 'echo' => true,));
		}
	}
	...
	public function _replyToSetTableFilterQuery($oQuery = null)
	{
		...
		foreach (get_object_taxonomies($GLOBALS['typenow']) as $sTaxonomySlug) {
			...
			if (is_object($oTerm)) $sVar = $oTerm->slug;
		}
		return $oQuery;
	}
...
if (!class_exists('WPBMAdminPageFramework_Message')) : class WPBMAdminPageFramework_Message
{
	...
	public static function instantiate($sTextDomain = 'admin-page-framework')
	{
		...
		if (!isset(self::$_oInstance) && !(self::$_oInstance instanceof WPBMAdminPageFramework_Message)) self::$_oInstance = new WPBMAdminPageFramework_Message($_sTextDomain);
		return self::$_oInstance;
	}
...
/* display: block; */
	...
	function __construct($oCaller, $sCallerPath, $sClassName)
	{
		...
		$this->sCallerPath = $sCallerPath ? $sCallerPath : WPBMAdminPageFramework_Utility::getCallerScriptPath(__FILE__);
		$this->sClassName = $sClassName;
		$this->sClassHash = md5($sClassName);
		$this->aScriptInfo = $this->getCallerInfo($this->sCallerPath);
		$GLOBALS['aWPBMAdminPageFramework'] = isset($GLOBALS['aWPBMAdminPageFramework']) && is_array($GLOBALS['aWPBMAdminPageFramework']) ? $GLOBALS['aWPBMAdminPageFramework'] : array('aFieldFlags' => array());
		$this->bIsAdmin = is_admin();
		$this->bIsMinifiedVersion = !class_exists('WPBMAdminPageFramework_Bootstrap');
		if (!isset(self::$_aLibraryData)) {
			$_sLibraryMainClassName = ($this->bIsMinifiedVersion) ? 'WPBMAdminPageFramework' : 'WPBMAdminPageFramework_Bootstrap';
	...
	{
		self::$_aLibraryData = WPBMAdminPageFramework_WPUtility::getScriptData($sLibraryFilePath, 'library');
	}

	static public function _getLibraryData($sLibraryFilePath = null)
	{
		if (isset(self::$_aLibraryData)) return self::$_aLibraryData;
		if ($sLibraryFilePath) self::_setLibraryData($sLibraryFilePath);
	...
	protected function getCallerInfo($sCallerPath = null)
	{
		...
		if ($aCallerInfo['sType'] == 'plugin') return WPBMAdminPageFramework_WPUtility::getScriptData($aCallerInfo['sPath'], $aCallerInfo['sType']) + $aCallerInfo;
	...
	public function isPostDefinitionPage($asPostTypes = array())
	{
		...
		if (isset($_GET['post'], $_GET['action']) && in_array($this->_sCurrentPostType, $_aPostTypes)) return true;
		return false;
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_Property_MetaBox')) : class WPBMAdminPageFramework_Property_MetaBox extends WPBMAdminPageFramework_Property_Base
{
...
if (!class_exists('WPBMAdminPageFramework_Property_Page')) : class WPBMAdminPageFramework_Property_Page extends WPBMAdminPageFramework_Property_Base
{
	...
	public function __construct($oCaller, $sCallerPath, $sClassName, $sOptionKey, $sCapability = 'manage_options')
	{
		...
		$GLOBALS['aWPBMAdminPageFramework']['aPageClasses'] = isset($GLOBALS['aWPBMAdminPageFramework']['aPageClasses']) && is_array($GLOBALS['aWPBMAdminPageFramework']['aPageClasses']) ? $GLOBALS['aWPBMAdminPageFramework']['aPageClasses'] : array();
		$GLOBALS['aWPBMAdminPageFramework']['aPageClasses'][$sClassName] = $oCaller;
	...
	{
		$sPageSlug = $sPageSlug ? $sPageSlug : (isset($_GET['page']) ? $_GET['page'] : '');
		return (array_key_exists(trim($sPageSlug), $this->aPages)) ? true : false;
	}
	...
	public function getDefaultOptions($aFields)
	{
		...
		foreach ($aFields as $_sSectionID => $_aFields) {
			foreach ($_aFields as $_sFieldID => $_aField) {
				...
				if (isset($_aField['section_id']) && $_aField['section_id'] != '_default') $_aDefaultOptions[$_aField['section_id']][$_sFieldID] = $_vDefault; else $_aDefaultOptions[$_sFieldID] = $_vDefault;
			}
		}
	...
	{
		$_aSubFields = WPBMAdminPageFramework_Utility::getIntegerElements($aField);
...
if (!class_exists('WPBMAdminPageFramework_Property_PostType')) : class WPBMAdminPageFramework_Property_PostType extends WPBMAdminPageFramework_Property_Base
{
...
if (!class_exists('WPBMAdminPageFramework_Property_MetaBox_Page')) : class WPBMAdminPageFramework_Property_MetaBox_Page extends WPBMAdminPageFramework_Property_MetaBox
{
	...
	function __construct($oCaller, $sClassName, $sCapability = 'manage_options')
	{
		...
		$GLOBALS['aWPBMAdminPageFramework']['aMetaBoxForPagesClasses'] = isset($GLOBALS['aWPBMAdminPageFramework']['aMetaBoxForPagesClasses']) && is_array($GLOBALS['aWPBMAdminPageFramework']['aMetaBoxForPagesClasses']) ? $GLOBALS['aWPBMAdminPageFramework']['aMetaBoxForPagesClasses'] : array();
		$GLOBALS['aWPBMAdminPageFramework']['aMetaBoxForPagesClasses'][$sClassName] = $oCaller;
	}
	...
	{
		return ($oAdminPage = $this->_getOwnerClass($sPageSlug)) ? $oAdminPage->oProp->aPages[$sPageSlug]['_page_hook'] : '';
	}

	public function isPageAdded($sPageSlug = '')
	{
		return ($oAdminPage = $this->_getOwnerClass($sPageSlug)) ? $oAdminPage->oProp->isPageAdded($sPageSlug) : false;
	}
	...
	{
		if (!$sPageSlug) return '';
		return ($oAdminPage = $this->_getOwnerClass($sPageSlug)) ? $oAdminPage->oProp->getDefaultInPageTab($sPageSlug) : '';
	}

	public function getOptionKey($sPageSlug)
	{
		if (!$sPageSlug) return '';
		return ($oAdminPage = $this->_getOwnerClass($sPageSlug)) ? $oAdminPage->oProp->sOptionKey : '';
	}

	private function _getOwnerClass($sPageSlug)
	{
		foreach ($GLOBALS['aWPBMAdminPageFramework']['aPageClasses'] as $oClass) if ($oClass->oProp->isPageAdded($sPageSlug)) return $oClass;
		return null;
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_Property_TaxonomyField')) : class WPBMAdminPageFramework_Property_TaxonomyField extends WPBMAdminPageFramework_Property_MetaBox
{
	public $_sPropertyType = 'taxonomy_field';
	public $aTaxonomySlugs;
	public $sOptionKey;
} endif;
if (!class_exists('WPBMAdminPageFramework_Script_AttributeUpdator')) : class WPBMAdminPageFramework_Script_AttributeUpdator
{
...
if (!class_exists('WPBMAdminPageFramework_Script_RegisterCallback')) : class WPBMAdminPageFramework_Script_RegisterCallback
{
	static public function getjQueryPlugin()
	{
		return "(function ( $ ) {
...
if (!class_exists('WPBMAdminPageFramework_Script_RepeatableField')) : class WPBMAdminPageFramework_Script_RepeatableField
{
...
if (!class_exists('WPBMAdminPageFramework_Script_RepeatableSection')) : class WPBMAdminPageFramework_Script_RepeatableSection
{
	static public function getjQueryPlugin($sCannotAddMore, $sCannotRemoveMore)
	{
		return "( function( $ ) {
...
if (!class_exists('WPBMAdminPageFramework_Script_Sortable')) : class WPBMAdminPageFramework_Script_Sortable
{
	static public function getjQueryPlugin()
	{
		return "(function($) {
...
if (!class_exists('WPBMAdminPageFramework_Script_Tab')) : class WPBMAdminPageFramework_Script_Tab
{
	static public function getjQueryPlugin()
	{
		return "( function( $ ) {
...
if (!class_exists('WPBMAdminPageFramework_Script_Utility')) : class WPBMAdminPageFramework_Script_Utility
{
	static public function getjQueryPlugin()
	{
		return "( function( $ ) {
...
if (!class_exists('WPBMAdminPageFramework_CustomSubmitFields')) : abstract class WPBMAdminPageFramework_CustomSubmitFields
{
...
if (!class_exists('WPBMAdminPageFramework_ExportOptions')) : class WPBMAdminPageFramework_ExportOptions extends WPBMAdminPageFramework_CustomSubmitFields
{
	...
	public function doExport($vData, $sFileName = null, $sFormatType = null)
	{
		...
		switch (strtolower($sFormatType)) {
			case 'text':
				if (is_array($vData) || is_object($vData)) die(WPBMAdminPageFramework_Debug::getArray($vData, null, false));
				die($vData);
			case 'json':
...
if (!class_exists('WPBMAdminPageFramework_ImportOptions')) : class WPBMAdminPageFramework_ImportOptions extends WPBMAdminPageFramework_CustomSubmitFields
{
...
if (!class_exists('WPBMAdminPageFramework_RegisterClasses')) : class WPBMAdminPageFramework_RegisterClasses
{
	...
	protected function composeClassArray($sClassDirPath, $aAllowedExtensions)
	{
		...
		$aClasses = array();
		foreach ($aFilePaths as $sFilePath) $aClasses[pathinfo($sFilePath, PATHINFO_FILENAME)] = $sFilePath;
		return $aClasses;
	}
	...
	protected function doRecursiveGlob($sPathPatten, $iFlags = 0)
	{
		...
		foreach (glob(dirname($sPathPatten) . '/*', GLOB_ONLYDIR | GLOB_NOSORT) as $sDirPath) $aFiles = array_merge($aFiles, $this->doRecursiveGlob($sDirPath . '/' . basename($sPathPatten), $iFlags));
		return $aFiles;
	}
	...
	{
		if (array_key_exists($sCalledUnknownClassName, $this->_aClasses) && file_exists($this->_aClasses[$sCalledUnknownClassName])) include_once($this->_aClasses[$sCalledUnknownClassName]);
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_Utility_Array')) : abstract class WPBMAdminPageFramework_Utility_Array
{
	public static function getCorrespondingArrayValue($vSubject, $sKey, $sDefault = '', $bBlankToDefault = false)
	{
		...
		if (!is_array($vSubject)) return ( string )$vSubject;
	...
	{
		$aMod = array();
		foreach ($aModel as $sKey => $_v) $aMod[$sKey] = isset($aSubject[$sKey]) ? $aSubject[$sKey] : null;
		return $aMod;
	}
	...
	public static function uniteArrays($aPrecedence, $aDefault1)
	{
		...
		$aArray = array();
		foreach ($aArgs as $aArg) $aArray = self::uniteArraysRecursive($aArg, $aArray);
		return $aArray;
	}
	...
	public static function uniteArraysRecursive($aPrecedence, $aDefault)
	{
		...
		foreach ($aDefault as $sKey => $v) {
			if (!array_key_exists($sKey, $aPrecedence) || is_null($aPrecedence[$sKey])) $aPrecedence[$sKey] = $v; else {
				if (is_array($aPrecedence[$sKey]) && is_array($v)) $aPrecedence[$sKey] = self::uniteArraysRecursive($aPrecedence[$sKey], $v);
			}
		}
		return $aPrecedence;
	}
	...
	static public function getIntegerElements($aParse)
	{
		...
		foreach ($aParse as $isKey => $v) {
			...
			$isKey = $isKey + 0;
			if (!is_int($isKey)) unset($aParse[$isKey]);
		}
		return $aParse;
	}
	...
	{
		foreach ($aParse as $isKey => $v) if (is_numeric($isKey) && is_int($isKey + 0)) unset($aParse[$isKey]);
		return $aParse;
	}
	...
	static public function numerizeElements($aSubject)
	{
		...
		foreach ($_aNumeric as &$_aElem) $_aElem = self::uniteArrays($_aElem, $_aAssociative);
		if (!empty($_aAssociative)) array_unshift($_aNumeric, $_aAssociative);
		return $_aNumeric;
	}
...
if (!class_exists('WPBMAdminPageFramework_Utility_String')) : abstract class WPBMAdminPageFramework_Utility_String extends WPBMAdminPageFramework_Utility_Array
{
...
if (!class_exists('WPBMAdminPageFramework_Utility_Path')) : abstract class WPBMAdminPageFramework_Utility_Path extends WPBMAdminPageFramework_Utility_String
{
...
if (!class_exists('WPBMAdminPageFramework_Utility_URL')) : abstract class WPBMAdminPageFramework_Utility_URL extends WPBMAdminPageFramework_Utility_Path
{
...
if (!class_exists('WPBMAdminPageFramework_Utility')) : abstract class WPBMAdminPageFramework_Utility extends WPBMAdminPageFramework_Utility_URL
{
	...
	static public function generateAttributes(array $aAttributes)
	{
		...
		foreach ($aAttributes as $sAttribute => $sProperty) {
			if (empty($sProperty) && $sProperty !== 0) continue;
...
if (!class_exists('WPBMAdminPageFramework_WPUtility_URL')) : class WPBMAdminPageFramework_WPUtility_URL extends WPBMAdminPageFramework_Utility
{
	public function getCurrentAdminURL()
	{
		...
		if ($_SERVER["SERVER_PORT"] != "80") $sPageURL .= $_SERVER["SERVER_NAME"] . ":" . $_SERVER["SERVER_PORT"] . $sRequestURI; else $sPageURL .= $_SERVER["SERVER_NAME"] . $sRequestURI;
		return $sPageURL;
	}
	...
	static public function getSRCFromPath($sFilePath)
	{
		...
		$sRelativePath = WPBMAdminPageFramework_Utility::getRelativePath(ABSPATH, $sFilePath);
	...
	{
		if (!$sSRC) return $bReturnNullIfNotExist ? null : $sSRC;
		if (filter_var($sSRC, FILTER_VALIDATE_URL)) return $sSRC;
		if (file_exists(realpath($sSRC))) return self::getSRCFromPath($sSRC);
		if ($bReturnNullIfNotExist) return null;
		return $sSRC;
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_WPUtility_HTML')) : class WPBMAdminPageFramework_WPUtility_HTML extends WPBMAdminPageFramework_WPUtility_URL
{
	static public function generateAttributes(array $aAttributes)
	{
...
if (!class_exists('WPBMAdminPageFramework_WPUtility_Page')) : class WPBMAdminPageFramework_WPUtility_Page extends WPBMAdminPageFramework_WPUtility_HTML
{
...
if (!class_exists('WPBMAdminPageFramework_WPUtility_Hook')) : class WPBMAdminPageFramework_WPUtility_Hook extends WPBMAdminPageFramework_WPUtility_Page
{
	...
	public function addAndApplyFilters()
	{
		$aArgs = func_get_args();
		$oCallerObject = $aArgs[0];
		$aFilters = $aArgs[1];
	...
	public function addAndApplyFilter()
	{
		$iArgs = func_num_args();
		$aArgs = func_get_args();
		$oCallerObject = $aArgs[0];
	...
	{
		$aFilters = array();
		if ($sTabSlug && $sPageSlug) $aFilters[] = "{$sPrefix}{$sPageSlug}_{$sTabSlug}";
		if ($sPageSlug) $aFilters[] = "{$sPrefix}{$sPageSlug}";
		if ($sClassName) $aFilters[] = "{$sPrefix}{$sClassName}";
		return $bReverse ? array_reverse($aFilters) : $aFilters;
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_WPUtility_File')) : class WPBMAdminPageFramework_WPUtility_File extends WPBMAdminPageFramework_WPUtility_Hook
{
...
if (!class_exists('WPBMAdminPageFramework_WPUtility_Option')) : class WPBMAdminPageFramework_WPUtility_Option extends WPBMAdminPageFramework_WPUtility_File
{
	static public function getOption($sOptionKey, $asKey = null, $vDefault = null)
	{
		if (!$asKey) {
...
if (!class_exists('WPBMAdminPageFramework_WPUtility')) : class WPBMAdminPageFramework_WPUtility extends WPBMAdminPageFramework_WPUtility_Option
{
} endif;
if (!class_exists('WPBMAdminPageFramework_FieldType_Base')) : abstract class WPBMAdminPageFramework_FieldType_Base extends WPBMAdminPageFramework_WPUtility
{
	...
	protected function enqueueMediaUploader()
	{
		...
		if (function_exists('wp_enqueue_media')) add_action('admin_footer', array($this, '_replyToEnqueueMedia'), 1); else wp_enqueue_script('media-upload');
		if (in_array($GLOBALS['pagenow'], array('media-upload.php', 'async-upload.php',))) add_filter('gettext', array($this, '_replyToReplaceThickBoxText'), 1, 2);
	}
	...
	public function _replyToRemovingMediaLibraryTab($aTabs)
	{
		...
		if (!$_REQUEST['enable_external_source']) unset($aTabs['type_url']);
		return $aTabs;
	}
	...
	protected function _getScript_CustomMediaUploaderObject()
	{
		...
		$GLOBALS['aWPBMAdminPageFramework']['aLoadedCustomMediaUploaderObject'] = isset($GLOBALS['aWPBMAdminPageFramework']['aLoadedCustomMediaUploaderObject']) ? $GLOBALS['aWPBMAdminPageFramework']['aLoadedCustomMediaUploaderObject'] : array();
		if (isset($GLOBALS['aWPBMAdminPageFramework']['aLoadedCustomMediaUploaderObject'][$this->_sFieldSetType])) return '';
		$GLOBALS['aWPBMAdminPageFramework']['aLoadedCustomMediaUploaderObject'][$this->_sFieldSetType] = true;
		return "
...
if (!class_exists('WPBMAdminPageFramework_FieldType')) : abstract class WPBMAdminPageFramework_FieldType extends WPBMAdminPageFramework_FieldType_Base
{
	public function _replyToFieldLoader()
	{
		$this->setUp();
...
if (!class_exists('WPBMAdminPageFramework_FormField_Base')) : class WPBMAdminPageFramework_FormField_Base extends WPBMAdminPageFramework_WPUtility
{
	...
	private function _loadScripts()
	{
		...
		if (!$_bIsLoadedRepeatable) $_bIsLoadedRepeatable = add_action('admin_footer', array($this, '_replyToAddRepeatableFieldjQueryPlugin'));
		if (!$_bIsLoadedSortable) $_bIsLoadedSortable = add_action('admin_footer', array($this, '_replyToAddSortableFieldPlugin'));
		if (!$_bIsLoadedRegisterCallback) $_bIsLoadedRegisterCallback = add_action('admin_footer', array($this, '_replyToAddRegisterCallbackjQueryPlugin'));
	}
	...
	{
		echo "<script type='text/javascript' class='admin-page-framework-repeatable-fields-plugin'>" . WPBMAdminPageFramework_Script_RepeatableField::getjQueryPlugin($this->oMsg->__('allowed_maximum_number_of_fields'), $this->oMsg->__('allowed_minimum_number_of_fields')) . "</script>";
	}

	public function _replyToAddAttributeUpdaterjQueryPlugin()
	{
		echo "<script type='text/javascript' class='admin-page-framework-attribute-updater'>" . WPBMAdminPageFramework_Script_AttributeUpdator::getjQueryPlugin() . "</script>";
	}

	public function _replyToAddRegisterCallbackjQueryPlugin()
	{
		echo "<script type='text/javascript' class='admin-page-framework-register-callback'>" . WPBMAdminPageFramework_Script_RegisterCallback::getjQueryPlugin() . "</script>";
	}

	public function _replyToAddUtilityPlugins()
	{
		echo "<script type='text/javascript' class='admin-page-framework-utility-plugins'>" . WPBMAdminPageFramework_Script_Utility::getjQueryPlugin() . "</script>";
	}

	public function _replyToAddSortableFieldPlugin()
	{
		wp_enqueue_script('jquery-ui-sortable');
		echo "<script type='text/javascript' class='admin-page-framework-sortable-field-plugin'>" . WPBMAdminPageFramework_Script_Sortable::getjQueryPlugin() . "</script>";
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_FormField')) : class WPBMAdminPageFramework_FormField extends WPBMAdminPageFramework_FormField_Base
{
	private function _getInputName($aField = null, $sKey = '')
	{
		...
		switch ($aField['_fields_type']) {
			default:
			case 'page':
				$sSectionDimension = isset($aField['section_id']) && $aField['section_id'] && $aField['section_id'] != '_default' ? "[{$aField['section_id']}]" : '';
				return "{$aField['option_key']}{$sSectionDimension}{$sSectionIndex}[{$aField['field_id']}]{$_sKey}";
			case 'page_meta_box':
			case 'post_meta_box':
				return isset($aField['section_id']) && $aField['section_id'] && $aField['section_id'] != '_default' ? "{$aField['section_id']}{$sSectionIndex}[{$aField['field_id']}]{$_sKey}" : "{$aField['field_id']}{$_sKey}";
			case 'taxonomy':
	...
	private function _getInputFieldValue($aField, $aOptions)
	{
		switch ($aField['_fields_type']) {
			...
			case 'page_meta_box':
			case 'taxonomy':
				if (!isset($aField['section_id']) || $aField['section_id'] == '_default') return isset($aOptions[$aField['field_id']]) ? $aOptions[$aField['field_id']] : null;
				if (isset($aField['_section_index'])) return isset($aOptions[$aField['section_id']][$aField['_section_index']][$aField['field_id']]) ? $aOptions[$aField['section_id']][$aField['_section_index']][$aField['field_id']] : null;
			...
			case 'post_meta_box':
				...
				if (!isset($aField['section_id']) || $aField['section_id'] == '_default') return get_post_meta($_GET['post'], $aField['field_id'], true);
				$aSectionArray = get_post_meta($_GET['post'], $aField['section_id'], true);
				if (isset($aField['_section_index'])) return isset($aSectionArray[$aField['_section_index']][$aField['field_id']]) ? $aSectionArray[$aField['_section_index']][$aField['field_id']] : null;
	...
	private function _getInputID($aField, $sIndex)
	{
		...
		$sFieldIndex = '__' . $sIndex;
		return isset($aField['section_id']) && $aField['section_id'] != '_default' ? $aField['section_id'] . $sSectionIndex . '_' . $aField['field_id'] . $sFieldIndex : $aField['field_id'] . $sFieldIndex;
	}

	static public function _getInputTagID($aField)
	{
		$sSectionIndex = isset($aField['_section_index']) ? '__' . $aField['_section_index'] : '';
		return isset($aField['section_id']) && $aField['section_id'] != '_default' ? $aField['section_id'] . $sSectionIndex . '_' . $aField['field_id'] : $aField['field_id'];
	}
	...
	public function _getFieldOutput()
	{
		$aFieldsOutput = array();
		$aExtraOutput = array();
		if (isset($this->aField['section_id'], $this->aErrors[$this->aField['section_id']], $this->aErrors[$this->aField['section_id']][$this->aField['field_id']])) $aFieldsOutput[] = "<span style='color:red;'>*&nbsp;{$this->aField['error_message']}" . $this->aErrors[$this->aField['section_id']][$this->aField['field_id']] . "</span><br />"; else if (isset($this->aErrors[$this->aField['field_id']])) $aFieldsOutput[] = "<span style='color:red;'>*&nbsp;{$this->aField['error_message']}" . $this->aErrors[$this->aField['field_id']] . "</span><br />";
		...
		foreach ($aFields as $sKey => $aField) {
			$aFieldTypeDefinition = isset($this->aFieldTypeDefinitions[$aField['type']]) ? $this->aFieldTypeDefinitions[$aField['type']] : $this->aFieldTypeDefinitions['default'];
			$aField['_index'] = $sKey;
			$aField['input_id'] = $this->_getInputID($aField, $sKey);
			$aField['_input_name'] = $this->_getInputName($this->aField, $aField['_is_multiple_fields'] ? $sKey : '');
			$aField['_input_name_flat'] = $this->_getFlatInputName($this->aField, $aField['_is_multiple_fields'] ? $sKey : '');
			...
			$aField['attributes'] = $this->uniteArrays(( array )$aField['attributes'], array('id' => $aField['input_id'], 'name' => $aField['_input_name'], 'value' => $aField['value'], 'type' => $aField['type'], 'disabled' => null,), ( array )$aFieldTypeDefinition['aDefaultKeys']['attributes']);
			$_aFieldAttributes = array('id' => $aField['_field_container_id'], 'class' => "admin-page-framework-field admin-page-framework-field-{$aField['type']}" . ($aField['attributes']['disabled'] ? ' disabled' : ''), 'data-type' => "{$aField['type']}",) + $aField['attributes']['field'];
			$aFieldsOutput[] = is_callable($aFieldTypeDefinition['hfRenderField']) ? $aField['before_field'] . "<div " . $this->generateAttributes($_aFieldAttributes) . ">" . call_user_func_array($aFieldTypeDefinition['hfRenderField'], array($aField)) . (($sDelimiter = $aField['delimiter']) ? "<div " . $this->generateAttributes(array('class' => 'delimiter', 'id' => "delimiter-{$aField['input_id']}", 'style' => $this->isLastElement($aFields, $sKey) ? "display:none;" : "",)) . ">{$sDelimiter}</div>" : "") . "</div>" . $aField['after_field'] : "";
		}
		$aExtraOutput[] = (isset($this->aField['description']) && trim($this->aField['description']) != '') ? "<p class='admin-page-framework-fields-description'><span class='description'>{$this->aField['description']}</span></p>" : '';
		$aExtraOutput[] = $this->aField['repeatable'] ? $this->_getRepeaterFieldEnablerScript('fields-' . $this->aField['tag_id'], count($aFields), $this->aField['repeatable']) : '';
		$aExtraOutput[] = $this->aField['sortable'] && (count($aFields) > 1 || $this->aField['repeatable']) ? $this->_getSortableFieldEnablerScript('fields-' . $this->aField['tag_id']) : '';
		$_aFieldsSetAttributes = array('id' => 'fieldset-' . $this->aField['tag_id'], 'class' => 'admin-page-framework-fieldset', 'data-field_id' => $this->aField['tag_id'],) + $this->aField['attributes']['fieldset'];
		$_aFieldsContainerAttributes = array('id' => 'fields-' . $this->aField['tag_id'], 'class' => 'admin-page-framework-fields' . ($this->aField['repeatable'] ? ' repeatable' : '') . ($this->aField['sortable'] ? ' sortable' : ''), 'data-type' => $this->aField['type'],) + $this->aField['attributes']['fields'];
		return "<fieldset " . $this->generateAttributes($_aFieldsSetAttributes) . ">" . "<div " . $this->generateAttributes($_aFieldsContainerAttributes) . ">" . $this->aField['before_fields'] . implode(PHP_EOL, $aFieldsOutput) . $this->aField['after_fields'] . "</div>" . implode(PHP_EOL, $aExtraOutput) . "</fieldset>";
	}
	...
	protected function _composeFieldsArray(&$aField, &$aOptions)
	{
		...
		foreach ($aField as $nsIndex => $vFieldElement) {
			if (is_numeric($nsIndex)) $aSubFields[] = $vFieldElement; else $aFirstField[$nsIndex] = $vFieldElement;
		}
		if ($aField['repeatable']) foreach (( array )$vSavedValue as $iIndex => $vValue) {
...
if (!class_exists('WPBMAdminPageFramework_FormTable_Base')) : class WPBMAdminPageFramework_FormTable_Base extends WPBMAdminPageFramework_WPUtility
{
	...
	private function _loadScripts()
	{
		...
		if (!$_bIsLoadedTabPlugin) $_bIsLoadedTabPlugin = add_action('admin_footer', array($this, '_replyToAddTabPlugin'));
	}
	...
	public function _replyToAddRepeatableSectionjQueryPlugin()
	{
		...
		if ($bIsCalled) return;
		$bIsCalled = true;
		echo "<script type='text/javascript' class='admin-page-framework-repeatable-sections-plugin'>" . WPBMAdminPageFramework_Script_RepeatableSection::getjQueryPlugin($this->oMsg->__('allowed_maximum_number_of_sections'), $this->oMsg->__('allowed_minimum_number_of_sections')) . "</script>";
	}

	public function _replyToAddTabPlugin()
	{
		echo "<script type='text/javascript' class='admin-page-framework-tab-plugin'>" . WPBMAdminPageFramework_Script_Tab::getjQueryPlugin() . "</script>";
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_FormTable')) : class WPBMAdminPageFramework_FormTable extends WPBMAdminPageFramework_FormTable_Base
{
	...
	public function getFormTable($sSectionTagID, $iSectionIndex, $aSection, $aFields, $hfSectionCallback, $hfFieldCallback)
	{
		...
		$aOutput = array();
		$aOutput[] = "<table " . $this->generateAttributes(array('id' => 'section_table-' . $sSectionTagID, 'class' => 'form-table',)) . ">" . ($aSection['description'] || $aSection['title'] ? "<caption class='admin-page-framework-section-caption' data-section_tab='{$aSection['section_tab_slug']}'>" . ($aSection['title'] && !$aSection['section_tab_slug'] ? "<div class='admin-page-framework-section-title' {$_sDisplayNone}>" . $this->_getSectionTitle($aSection['title'], 'h3', $aFields, $hfFieldCallback) . "</div>" : "") . ($aSection['description'] && is_callable($hfSectionCallback) ? "<div class='admin-page-framework-section-description'>" . call_user_func_array($hfSectionCallback, array('<p>' . $aSection['description'] . '</p>', $aSection)) . "</div>" : "") . "</caption>" : "<caption class='admin-page-framework-section-caption' style='display:none;'></caption>") . $this->getFieldRows($aFields, $hfFieldCallback) . "</table>";
		return "<div " . $this->generateAttributes(array('id' => $sSectionTagID, 'class' => 'admin-page-framework-section' . ($aSection['section_tab_slug'] ? ' admin-page-framework-tab-content' : ''),)) . ">" . implode(PHP_EOL, $aOutput) . "</div>";
	}
	...
	protected function _getFieldRow($aField, $hfCallback)
	{
		...
		$_sAttributes = $this->_getAttributes($_aField, array('id' => 'fieldrow-' . WPBMAdminPageFramework_FormField::_getInputTagID($_aField), 'valign' => 'top', 'class' => 'admin-page-framework-fieldrow',));
		$aOutput[] = "<tr {$_sAttributes}>";
		if ($_aField['show_title_column']) $aOutput[] = "<th>" . $this->_getFieldTitle($_aField) . "</th>";
	...
	protected function _getField($aField, $hfCallback)
	{
		...
		if ($_aField['show_title_column']) $aOutput[] = $this->_getFieldTitle($_aField);
		$aOutput[] = call_user_func_array($hfCallback, array($aField));
		$aOutput[] = "</div>";
		return implode(PHP_EOL, $aOutput);
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_Link_Base')) : abstract class WPBMAdminPageFramework_Link_Base extends WPBMAdminPageFramework_WPUtility
{
	...
	protected function _setFooterInfoRight($aScriptInfo, &$sFooterInfoRight)
	{
		...
		$sFooterInfoRight = $this->oMsg->__('powered_by') . '&nbsp;' . $sLibraryInfo . ", <a href='http://wordpress.org' target='_blank' title='WordPress {$GLOBALS['wp_version']}'>WordPress</a>";
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_FormElement_Utility')) : class WPBMAdminPageFramework_FormElement_Utility extends WPBMAdminPageFramework_WPUtility
{
	public function dropRepeatableElements(array $aOptions)
	{
		foreach ($aOptions as $_sFieldOrSectionID => $_aSectionOrFieldValue) {
			...
			if ($this->isRepeatableField($_sFieldID, '_default')) unset($aOptions[$_sFieldID]);
		}
		return $aOptions;
	}
	...
	public function getFieldsModel(array $aFields = array())
	{
		...
		foreach ($aFields as $_sSectionID => $_aFields) {
			...
			foreach ($_aFields as $_sFieldID => $_aField) $_aFieldsModel[$_aField['field_id']] = $_aField;
		}
...
if (!class_exists('WPBMAdminPageFramework_FormElement')) : class WPBMAdminPageFramework_FormElement extends WPBMAdminPageFramework_FormElement_Utility
{
	...
	public function addField($asField)
	{
		...
		if (!isset($aField['field_id'], $aField['type'])) return null;
	...
	public function removeField($sFieldID)
	{
		foreach ($this->aFields as $_sSectionID => $_aSubSectionsOrFields) {
			if (array_key_exists($sFieldID, $_aSubSectionsOrFields)) unset($this->aFields[$_sSectionID][$sFieldID]);
			foreach ($_aSubSectionsOrFields as $_sIndexOrFieldID => $_aSubSectionOrFields) {
				if (is_numeric($_sIndexOrFieldID) && is_int($_sIndexOrFieldID + 0)) {
					if (array_key_exists($sFieldID, $_aSubSectionOrFields)) unset($this->aFields[$_sSectionID][$_sIndexOrFieldID]);
					continue;
				}
			}
		}
	...
	public function formatFields($sFieldsType, $sCapability)
	{
		...
		foreach ($this->aFields as $_sSectionID => $_aSubSectionsOrFields) {
			...
			if (count($this->getIntegerElements($_aSubSectionsOrFields)) || $_abSectionRepeatable) {
				foreach ($this->numerizeElements($_aSubSectionsOrFields) as $_iSectionIndex => $_aFields) {
					foreach ($_aFields as $_aField) {
						...
						if ($_aField) $_aNewFields[$_sSectionID][$_iSectionIndex][$_aField['field_id']] = $_aField;
					}
			...
			foreach ($_aSectionedFields as $_sFieldID => $_aField) {
				...
				if ($_aField) $_aNewFields[$_sSectionID][$_aField['field_id']] = $_aField;
			}
			uasort($_aNewFields[$_sSectionID], array($this, '_sortByOrder'));
		}
		if (!empty($this->aSections) && !empty($_aNewFields)) : $_aSortedFields = array();
			foreach ($this->aSections as $sSectionID => $aSeciton) if (isset($_aNewFields[$sSectionID])) $_aSortedFields[$sSectionID] = $_aNewFields[$sSectionID];
			$_aNewFields = $_aSortedFields; endif;
	...
	public function getConditionedSections($aSections = null)
	{
		...
		foreach ($aSections as $_sSectionID => $_aSection) {
			...
			if ($_aSection) $aNewSections[$_sSectionID] = $_aSection;
		}
			...
			foreach ($_aSubSectionOrFields as $_sIndexOrFieldID => $_aSubSectionOrField) {
				if (is_numeric($_sIndexOrFieldID) && is_int($_sIndexOrFieldID + 0)) {
					...
					foreach ($_aFields as $_aField) {
						...
						if ($_aField) $_aNewFields[$_sSectionID][$_sSubSectionIndex][$_aField['field_id']] = $_aField;
					}
					continue;
				}
				$_aField = $_aSubSectionOrField;
	...
	public function setDynamicElements($aOptions)
	{
		...
		foreach ($aOptions as $_sSectionID => $_aSubSectionOrFields) {
			...
			foreach ($_aSubSectionOrFields as $_isIndexOrFieldID => $_aSubSectionOrFieldOptions) {
				...
				foreach ($_aSubSection[$_iIndex] as &$_aField) $_aField['_section_index'] = $_iIndex;
				unset($_aField);
				$_iPrevIndex = $_iIndex;
			}
			if (!empty($_aSubSection)) $this->aConditionedFields[$_sSectionID] = $_aSubSection;
		}
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_FieldType_checkbox')) : class WPBMAdminPageFramework_FieldType_checkbox extends WPBMAdminPageFramework_FieldType_Base
{
	...
	public function _replyToGetField($aField)
	{
		...
		foreach (( array )$aField['label'] as $sKey => $sLabel) {
			$aInputAttributes = array('type' => 'checkbox', 'id' => $aField['input_id'] . '_' . $sKey, 'checked' => $this->getCorrespondingArrayValue($asValue, $sKey, null) == 1 ? 'checked' : '', 'value' => 1, 'name' => is_array($aField['label']) ? "{$aField['attributes']['name']}[{$sKey}]" : $aField['attributes']['name'],) + $this->getFieldElementByKey($aField['attributes'], $sKey, $aField['attributes']) + $aField['attributes'];
			$aLabelAttributes = array('for' => $aInputAttributes['id'], 'class' => $aInputAttributes['disabled'] ? 'disabled' : '',);
			$aOutput[] = $this->getFieldElementByKey($aField['before_label'], $sKey) . "<div class='admin-page-framework-input-label-container admin-page-framework-checkbox-label' style='min-width: {$aField['label_min_width']}px;'>" . "<label " . $this->generateAttributes($aLabelAttributes) . ">" . $this->getFieldElementByKey($aField['before_input'], $sKey) . "<span class='admin-page-framework-input-container'>" . "<input type='hidden' name='{$aInputAttributes['name']}' value='0' />" . "<input " . $this->generateAttributes($aInputAttributes) . " />" . "</span>" . "<span class='admin-page-framework-input-label-string'>" . $sLabel . "</span>" . $this->getFieldElementByKey($aField['after_input'], $sKey) . "</label>" . "</div>" . $this->getFieldElementByKey($aField['after_label'], $sKey);
		}
		return implode(PHP_EOL, $aOutput);
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_FieldType_color')) : class WPBMAdminPageFramework_FieldType_color extends WPBMAdminPageFramework_FieldType_Base
{
	...
	public function _replyToGetField($aField)
	{
		...
		return $aField['before_label'] . "<div class='admin-page-framework-input-label-container'>" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . ($aField['label'] && !$aField['repeatable'] ? "<span class='admin-page-framework-input-label-string' style='min-width:" . $aField['label_min_width'] . "px;'>" . $aField['label'] . "</span>" : "") . "<input " . $this->generateAttributes($aField['attributes']) . " />" . $aField['after_input'] . "<div class='repeatable-field-buttons'></div>" . "</label>" . "<div class='colorpicker' id='color_{$aField['input_id']}'></div>" . $this->_getColorPickerEnablerScript("{$aField['input_id']}") . "</div>" . $aField['after_label'];
	}
...
if (!class_exists('WPBMAdminPageFramework_FieldType_default')) : class WPBMAdminPageFramework_FieldType_default extends WPBMAdminPageFramework_FieldType_Base
{
	...
	public function _replyToGetField($aField)
	{
		return $aField['before_label'] . "<div class='admin-page-framework-input-label-container'>" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . ($aField['label'] && !$aField['repeatable'] ? "<span class='admin-page-framework-input-label-string' style='min-width:" . $aField['label_min_width'] . "px;'>" . $aField['label'] . "</span>" : "") . $aField['value'] . $aField['after_input'] . "</label>" . "</div>" . $aField['after_label'];
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_FieldType_hidden')) : class WPBMAdminPageFramework_FieldType_hidden extends WPBMAdminPageFramework_FieldType_Base
{
	...
	public function _replyToGetField($aField)
	{
		return $aField['before_label'] . "<div class='admin-page-framework-input-label-container'>" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . ($aField['label'] ? "<span class='admin-page-framework-input-label-string' style='min-width:" . $aField['label_min_width'] . "px;'>" . $aField['label'] . "</span>" : "") . "<input " . $this->generateAttributes($aField['attributes']) . " />" . $aField['after_input'] . "</label>" . "</div>" . $aField['after_label'];
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_FieldType_image')) : class WPBMAdminPageFramework_FieldType_image extends WPBMAdminPageFramework_FieldType_Base
{
	public $aFieldTypeSlugs = array('image',);
	protected $aDefaultKeys = array('attributes_to_store' => array(), 'show_preview' => true, 'allow_external_source' => true, 'attributes' => array('input' => array('size' => 40, 'maxlength' => 400,), 'button' => array(), 'preview' => array(),),);
	...
	public function _replyToGetScripts()
	{
		return $this->_getScript_CustomMediaUploaderObject() . PHP_EOL . $this->_getScript_ImageSelector("admin_page_framework", $this->oMsg->__('upload_image'), $this->oMsg->__('use_this_image')) . PHP_EOL . $this->_getScript_RegisterCallbacks();
	}
...
console.log( 'input id: ' + sInputID );
	...
	public function _replyToGetField($aField)
	{
		...
		$sImageURL = $sCaptureAttribute ? (isset($aField['attributes']['value'][$sCaptureAttribute]) ? $aField['attributes']['value'][$sCaptureAttribute] : "") : $aField['attributes']['value'];
...
if (!class_exists('WPBMAdminPageFramework_FieldType_radio')) : class WPBMAdminPageFramework_FieldType_radio extends WPBMAdminPageFramework_FieldType_Base
{
	...
	public function _replyToGetField($aField)
	{
		...
		foreach ($aField['label'] as $sKey => $sLabel) {
			$aInputAttributes = array('type' => 'radio', 'checked' => $sValue == $sKey ? 'checked' : '', 'value' => $sKey, 'id' => $aField['input_id'] . '_' . $sKey, 'data-default' => $aField['default'],) + $this->getFieldElementByKey($aField['attributes'], $sKey, $aField['attributes']) + $aField['attributes'];
			$aLabelAttributes = array('for' => $aInputAttributes['id'], 'class' => $aInputAttributes['disabled'] ? 'disabled' : '',);
			$aOutput[] = $this->getFieldElementByKey($aField['before_label'], $sKey) . "<div class='admin-page-framework-input-label-container admin-page-framework-radio-label' style='min-width: {$aField['label_min_width']}px;'>" . "<label " . $this->generateAttributes($aLabelAttributes) . ">" . $this->getFieldElementByKey($aField['before_input'], $sKey) . "<span class='admin-page-framework-input-container'>" . "<input " . $this->generateAttributes($aInputAttributes) . " />" . "</span>" . "<span class='admin-page-framework-input-label-string'>" . $sLabel . "</span>" . $this->getFieldElementByKey($aField['after_input'], $sKey) . "</label>" . "</div>" . $this->getFieldElementByKey($aField['after_label'], $sKey);
		}
...
if (!class_exists('WPBMAdminPageFramework_FieldType_section_title')) : class WPBMAdminPageFramework_FieldType_section_title extends WPBMAdminPageFramework_FieldType_Base
{
...
if (!class_exists('WPBMAdminPageFramework_FieldType_select')) : class WPBMAdminPageFramework_FieldType_select extends WPBMAdminPageFramework_FieldType_Base
{
	public $aFieldTypeSlugs = array('select');
	protected $aDefaultKeys = array('is_multiple' => '', 'attributes' => array('select' => array('size' => 1, 'autofocusNew' => '', 'multiple' => '', 'required' => '',), 'optgroup' => array(), 'option' => array(),),);
	...
	public function _replyToGetField($aField)
	{
		$aSelectAttributes = array('id' => $aField['input_id'], 'multiple' => $aField['is_multiple'] ? 'multiple' : $aField['attributes']['select']['multiple'],) + $aField['attributes']['select'];
		$aSelectAttributes['name'] = empty($aSelectAttributes['multiple']) ? $aField['_input_name'] : "{$aField['_input_name']}[]";
		return $aField['before_label'] . "<div class='admin-page-framework-input-label-container admin-page-framework-select-label' style='min-width: {$aField['label_min_width']}px;'>" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . "<span class='admin-page-framework-input-container'>" . "<select " . $this->generateAttributes($aSelectAttributes) . " >" . $this->_getOptionTags($aField['input_id'], $aField['attributes'], $aField['label']) . "</select>" . "</span>" . $aField['after_input'] . "<div class='repeatable-field-buttons'></div>" . "</label>" . "</div>" . $aField['after_label'];
	}
	...
	protected function _getOptionTags($sInputID, &$aAttributes, $aLabel)
	{
		...
		foreach ($aLabel as $sKey => $asLabel) {
			if (is_array($asLabel)) {
				$aOptGroupAttributes = isset($aAttributes['optgroup'][$sKey]) && is_array($aAttributes['optgroup'][$sKey]) ? $aAttributes['optgroup'][$sKey] + $aAttributes['optgroup'] : $aAttributes['optgroup'];
				$aOutput[] = "<optgroup label='{$sKey}'" . $this->generateAttributes($aOptGroupAttributes) . ">" . $this->_getOptionTags($sInputID, $aAttributes, $asLabel) . "</optgroup>";
				continue;
			}
			$aValue = isset($aAttributes['option'][$sKey]['value']) ? $aAttributes['option'][$sKey]['value'] : $aValue;
			$aOptionAttributes = array('id' => $sInputID . '_' . $sKey, 'value' => $sKey, 'selected' => in_array(( string )$sKey, $aValue) ? 'Selected' : '',) + (isset($aAttributes['option'][$sKey]) && is_array($aAttributes['option'][$sKey]) ? $aAttributes['option'][$sKey] + $aAttributes['option'] : $aAttributes['option']);
			$aOutput[] = "<option " . $this->generateAttributes($aOptionAttributes) . " >" . $asLabel . "</option>";
		}
		return implode(PHP_EOL, $aOutput);
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_FieldType_submit')) : class WPBMAdminPageFramework_FieldType_submit extends WPBMAdminPageFramework_FieldType_Base
{
	...
	public function _replyToGetField($aField)
	{
		...
		$aInputAttributes = array('type' => 'submit', 'value' => ($sValue = $this->_getInputFieldValueFromLabel($aField)),) + $aField['attributes'] + array('title' => $sValue,);
		$aLabelAttributes = array('style' => $aField['label_min_width'] ? "min-width:{$aField['label_min_width']}px;" : null, 'for' => $aInputAttributes['id'], 'class' => $aInputAttributes['disabled'] ? 'disabled' : '',);
		$aLabelContainerAttributes = array('style' => $aField['label_min_width'] ? "min-width:{$aField['label_min_width']}px;" : null, 'class' => 'admin-page-framework-input-label-container admin-page-framework-input-button-container admin-page-framework-input-container',);
		return $aField['before_label'] . "<div " . $this->generateAttributes($aLabelContainerAttributes) . ">" . $this->_getExtraFieldsBeforeLabel($aField) . "<label " . $this->generateAttributes($aLabelAttributes) . ">" . $aField['before_input'] . $this->_getExtraInputFields($aField) . "<input " . $this->generateAttributes($aInputAttributes) . " />" . $aField['after_input'] . "</label>" . "</div>" . $aField['after_label'];
	}
	...
	private function _checkConfirmationDisplayed($sResetKey, $sFlatFieldName)
	{
		...
		$bResetConfirmed = get_transient(md5("reset_confirm_" . $sFlatFieldName)) !== false ? true : false;
		if ($bResetConfirmed) delete_transient(md5("reset_confirm_" . $sFlatFieldName));
...
if (!class_exists('WPBMAdminPageFramework_FieldType_export')) : class WPBMAdminPageFramework_FieldType_export extends WPBMAdminPageFramework_FieldType_submit
{
	...
	public function _replyToGetField($aField)
	{
		if (isset($aField['data'])) set_transient(md5("{$aField['class_name']}_{$aField['input_id']}"), $aField['data'], 60 * 2);
...
if (!class_exists('WPBMAdminPageFramework_FieldType_import')) : class WPBMAdminPageFramework_FieldType_import extends WPBMAdminPageFramework_FieldType_submit
{
...
if (!class_exists('WPBMAdminPageFramework_FieldType_taxonomy')) : class WPBMAdminPageFramework_FieldType_taxonomy extends WPBMAdminPageFramework_FieldType_Base
{
	public $aFieldTypeSlugs = array('taxonomy',);
	protected $aDefaultKeys = array('taxonomy_slugs' => 'category', 'height' => '250px', 'max_width' => '100$', 'attributes' => array(),);
...
if (!class_exists('WPBMAdminPageFramework_FieldType_text')) : class WPBMAdminPageFramework_FieldType_text extends WPBMAdminPageFramework_FieldType_Base
{
	public $aFieldTypeSlugs = array('text', 'password', 'date', 'datetime', 'datetime-local', 'email', 'month', 'search', 'tel', 'url', 'week',);
	protected $aDefaultKeys = array('attributes' => array('size' => 30, 'maxlength' => 400,),);
...
if (!class_exists('WPBMAdminPageFramework_FieldType_file')) : class WPBMAdminPageFramework_FieldType_file extends WPBMAdminPageFramework_FieldType_text
{
...
if (!class_exists('WPBMAdminPageFramework_FieldType_number')) : class WPBMAdminPageFramework_FieldType_number extends WPBMAdminPageFramework_FieldType_text
{
...
if (!class_exists('WPBMAdminPageFramework_FieldType_textarea')) : class WPBMAdminPageFramework_FieldType_textarea extends WPBMAdminPageFramework_FieldType_Base
{
...
if (!class_exists('WPBMAdminPageFramework_Link_Page')) : class WPBMAdminPageFramework_Link_Page extends WPBMAdminPageFramework_Link_Base
{
	private $oProp;
	...
	public function __construct(&$oProp, $oMsg = null)
	{
		...
		$aLibraryData = WPBMAdminPageFramework_Property_Base::_getLibraryData();
		...
		if ($this->oProp->aScriptInfo['sType'] == 'plugin') add_filter('plugin_action_links_' . plugin_basename($this->oProp->aScriptInfo['sPath']), array($this, '_replyToAddSettingsLinkInPluginListingPage'));
	}

	public function _addLinkToPluginDescription($linkss)
	{
		if (!is_array($linkss)) $this->oProp->aPluginDescriptionLinks[] = $linkss; else $this->oProp->aPluginDescriptionLinks = array_merge($this->oProp->aPluginDescriptionLinks, $linkss);
	...
	{
		if (!is_array($linkss)) $this->oProp->aPluginTitleLinks[] = $linkss; else $this->oProp->aPluginTitleLinks = array_merge($this->oProp->aPluginTitleLinks, $linkss);
	...
	{
		if (count($this->oProp->aPages) < 1) return $aLinks;
		$sLinkURL = preg_match('/^.+\.php/', $this->oProp->aRootMenu['sPageSlug']) ? add_query_arg(array('page' => $this->oProp->sDefaultPageSlug), admin_url($this->oProp->aRootMenu['sPageSlug'])) : "admin.php?page={$this->oProp->sDefaultPageSlug}";
		array_unshift($aLinks, '<a href="' . $sLinkURL . '">' . $this->oMsg->__('settings') . '</a>');
		return $aLinks;
	}
	...
	public function _replyToAddLinkToPluginDescription($aLinks, $sFile)
	{
		...
		$aAddingLinks = array();
		foreach ($this->oProp->aPluginDescriptionLinks as $linksHTML) if (is_array($linksHTML)) $aAddingLinks = array_merge($linksHTML, $aAddingLinks); else $aAddingLinks[] = ( string )$linksHTML;
	...
	{
		$aAddingLinks = array();
		foreach ($this->oProp->aPluginTitleLinks as $linksHTML) if (is_array($linksHTML)) $aAddingLinks = array_merge($linksHTML, $aAddingLinks); else $aAddingLinks[] = ( string )$linksHTML;
		return array_merge($aLinks, $aAddingLinks);
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_Link_PostType')) : class WPBMAdminPageFramework_Link_PostType extends WPBMAdminPageFramework_Link_Base
{
	public $aFooterInfo = array('sLeft' => '', 'sRight' => '',);
	...
	public function __construct($oProp, $oMsg = null)
	{
		...
		if ($this->oProp->aScriptInfo['sType'] == 'plugin') add_filter('plugin_action_links_' . plugin_basename($this->oProp->aScriptInfo['sPath']), array($this, '_replyToAddSettingsLinkInPluginListingPage'), 20);
		if (isset($_GET['post_type']) && $_GET['post_type'] == $this->oProp->sPostType) add_action('get_edit_post_link', array($this, '_replyToAddPostTypeQueryInEditPostLink'), 10, 3);
	}
	...
	public function _replyToAddSettingsLinkInPluginListingPage($aLinks)
	{
		array_unshift($aLinks, "<a href='edit.php?post_type={$this->oProp->sPostType}'>" . $this->sSettingPageLinkTitle . "</a>");
		return $aLinks;
	}

	public function _replyToAddInfoInFooterLeft($sLinkHTML = '')
	{
		if (!$this->isPostDefinitionPage($this->oProp->sPostType) && !$this->isPostListingPage($this->oProp->sPostType)) return $sLinkHTML;
		if (empty($this->oProp->aScriptInfo['sName'])) return $sLinkHTML;
...
if (!class_exists('WPBMAdminPageFramework_FormElement_Page')) : class WPBMAdminPageFramework_FormElement_Page extends WPBMAdminPageFramework_FormElement
{
		...
		foreach ($this->aFields as $_sSectionID => $_aFields) {
			...
			foreach ($_aFields as $_sFieldID => $_aField) {
				...
				if (is_numeric($_sFieldID) && is_int($_sFieldID + 0)) {
					if (array_key_exists($_sSectionID, $aOptions)) $_aStoredOptionsOfThePage[$_sSectionID] = $aOptions[$_sSectionID];
					continue;
				}
				...
				if (array_key_exists($_aField['field_id'], $aOptions)) $_aStoredOptionsOfThePage[$_aField['field_id']] = $aOptions[$_aField['field_id']];
			}
		}
	...
	public function getOtherPageOptions($aOptions, $sPageSlug)
	{
		...
		foreach ($this->aFields as $_sSectionID => $_aFields) {
			...
			foreach ($_aFields as $_sFieldID => $_aField) {
				...
				if (is_numeric($_sFieldID) && is_int($_sFieldID + 0)) continue;
				if (isset($_aField['section_id']) && $_aField['section_id'] != '_default') {
				...
				if (array_key_exists($_aField['field_id'], $aOptions)) $_aStoredOptionsNotOfThePage[$_aField['field_id']] = $aOptions[$_aField['field_id']];
			}
		}
	...
	public function getOtherTabOptions($aOptions, $sPageSlug, $sTabSlug)
	{
		...
		foreach ($this->aFields as $_sSectionID => $_aSubSectionsOrFields) {
			if (isset($this->aSections[$_sSectionID]['page_slug']) && $this->aSections[$_sSectionID]['page_slug'] == $sPageSlug && isset($this->aSections[$_sSectionID]['tab_slug']) && $this->aSections[$_sSectionID]['tab_slug'] == $sTabSlug) continue;
			foreach ($_aSubSectionsOrFields as $_isSubSectionIndexOrFieldID => $_aSubSectionOrField) {
				if (is_numeric($_isSubSectionIndexOrFieldID) && is_int($_isSubSectionIndexOrFieldID + 0)) {
					if (array_key_exists($_sSectionID, $aOptions)) $_aStoredOptionsNotOfTheTab[$_sSectionID] = $aOptions[$_sSectionID];
					continue;
				}
				$_aField = $_aSubSectionOrField;
				if (isset($_aField['section_id']) && $_aField['section_id'] != '_default') {
					if (array_key_exists($_aField['section_id'], $aOptions)) $_aStoredOptionsNotOfTheTab[$_aField['section_id']] = $aOptions[$_aField['section_id']];
					continue;
				}
				if (array_key_exists($_aField['field_id'], $aOptions)) $_aStoredOptionsNotOfTheTab[$_aField['field_id']] = $aOptions[$_aField['field_id']];
			}
		}
	...
	public function getTabOnlyOptions($aOptions, $sPageSlug, $sTabSlug = '')
	{
		...
		foreach ($this->aFields as $_sSectionID => $_aSubSectionsOrFields) {
			...
			foreach ($_aSubSectionsOrFields as $_sFieldID => $_aField) {
				if (is_numeric($_sFieldID) && is_int($_sFieldID + 0)) {
					if (array_key_exists($_sSectionID, $aOptions)) $_aStoredOptionsOfTheTab[$_sSectionID] = $aOptions[$_sSectionID];
					continue;
				}
				if (isset($_aField['section_id']) && $_aField['section_id'] != '_default') {
					if (array_key_exists($_aField['section_id'], $aOptions)) $_aStoredOptionsOfTheTab[$_aField['section_id']] = $aOptions[$_aField['section_id']];
					continue;
				}
				if (array_key_exists($_aField['field_id'], $aOptions)) $_aStoredOptionsOfTheTab[$_aField['field_id']] = $aOptions[$_aField['field_id']];
			}
		}
		return $_aStoredOptionsOfTheTab;
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_FieldType_posttype')) : class WPBMAdminPageFramework_FieldType_posttype extends WPBMAdminPageFramework_FieldType_checkbox
{
	public $aFieldTypeSlugs = array('posttype',);
	protected $aDefaultKeys = array('slugs_to_remove' => null, 'attributes' => array('size' => 30, 'maxlength' => 400,),);
	...
	{
		foreach (get_post_types('', 'objects') as $oPostType) if (isset($oPostType->name, $oPostType->label)) $aPostTypes[$oPostType->name] = $oPostType->label;
		return array_diff_key($aPostTypes, array_flip($aRemoveNames));
	}
} endif;
if (!class_exists('WPBMAdminPageFramework_FieldType_media')) : class WPBMAdminPageFramework_FieldType_media extends WPBMAdminPageFramework_FieldType_image
{
	...
	public function _replyToGetScripts()
	{
		return $this->_getScript_CustomMediaUploaderObject() . PHP_EOL . $this->_getScript_MediaUploader("admin_page_framework", $this->oMsg->__('upload_file'), $this->oMsg->__('use_this_file')) . PHP_EOL . $this->_getScript_RegisterCallbacks();
	}
...
if (!class_exists('WPBMAdminPageFramework_FieldType_size')) : class WPBMAdminPageFramework_FieldType_size extends WPBMAdminPageFramework_FieldType_select
{
